...
fn setup() -> (u16, Sidecar) {
    ...
    s.send_ok(
        "s",
        "start_tcp_slave",
        json!({ "slaveId": "t", "port": port, "allowedStationIds": [] }),
    );
...
fn e2e_fc03() {
    ...
    s.send_ok(
        "set",
        "slave_set_value",
        json!({ "slaveId": "t", "area": "holding", "address": 100, "values": [0x1234, 0xABCD] }),
    );
    let r = s.send_ok(
        "r",
        "tcp_read_holding_registers",
        json!({ "connectionId": "c", "startAddress": 100, "quantity": 2 }),
    );
...
fn e2e_fc04() {
    ...
    s.send_ok(
        "set",
        "slave_set_value",
        json!({ "slaveId": "t", "area": "input", "address": 10, "values": [1, 2, 3] }),
    );
    let r = s.send_ok(
        "r",
        "tcp_read_input_registers",
        json!({ "connectionId": "c", "startAddress": 10, "quantity": 3 }),
    );
...
fn e2e_fc01_padding() {
    ...
    s.send_ok(
        "set",
        "slave_set_coil",
        json!({ "slaveId": "t", "area": "coil", "address": 0, "values": vals }),
    );
    let r = s.send_ok(
        "r",
        "tcp_read_coils",
        json!({ "connectionId": "c", "startAddress": 0, "quantity": 12 }),
    );
    let coils = r["coils"].as_array().unwrap();
    assert_eq!(coils.len(), 12, "填充位必须截断: 实际 {}", coils.len());
    for i in 0..12 {
        assert_eq!(coils[i].as_bool(), Some(i % 2 == 0), "线圈 {}", i);
    }
}
...
fn e2e_fc02() {
    ...
    let r = s.send_ok(
        ...
        json!({ "connectionId": "c", "startAddress": 0, "quantity": 8 }),
    );
    let coils = r["coils"].as_array().unwrap();
...
fn e2e_fc05() {
    ...
    s.send_ok(
        ...
        json!({ "connectionId": "c", "address": 50, "value": true }),
    );
    let r = s.send_ok(
        "v1",
        "tcp_read_coils",
        json!({ "connectionId": "c", "startAddress": 50, "quantity": 1 }),
    );
    assert_eq!(r["coils"][0].as_bool(), Some(true));
    s.send_ok(
        "off",
        "tcp_write_single_coil",
        json!({ "connectionId": "c", "address": 50, "value": false }),
    );
    let r = s.send_ok(
        "v2",
        "tcp_read_coils",
...
fn e2e_fc06() {
    ...
    s.send_ok(
        ...
        json!({ "connectionId": "c", "address": 200, "value": 48879 }),
    );
    let r = s.send_ok(
        "r",
        "tcp_read_holding_registers",
        json!({ "connectionId": "c", "startAddress": 200, "quantity": 1 }),
    );
...
fn e2e_fc16() {
    ...
    let r = s.send_ok(
        "r",
        "tcp_read_holding_registers",
        json!({ "connectionId": "c", "startAddress": 300, "quantity": 5 }),
    );
    let regs = r["registers"].as_array().unwrap();
...
fn e2e_fc15() {
    ...
    let r = s.send_ok(
        "r",
        "tcp_read_coils",
        json!({ "connectionId": "c", "startAddress": 60, "quantity": 8 }),
    );
    let coils = r["coils"].as_array().unwrap();
    assert_eq!(coils.len(), 8);
    for i in 0..8 {
        assert_eq!(coils[i].as_bool(), Some(i % 2 == 0));
    }
}
...
fn e2e_fc22_formula() {
    ...
    s.send_ok(
        "init",
        "slave_set_value",
        json!({ "slaveId": "t", "area": "holding", "address": 400, "values": [0x000F] }),
    );
    ...
    let r = s.send_ok(
        "v",
        "tcp_read_holding_registers",
        json!({ "connectionId": "c", "startAddress": 400, "quantity": 1 }),
    );
    assert_eq!(
        r["registers"][0].as_u64(),
        Some(0x000F),
