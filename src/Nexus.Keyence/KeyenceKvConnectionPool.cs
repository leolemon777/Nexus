...
{
    /// <summary>
    /// Keyence KV 连接池 — 复用 TCP 连接，适合高频寄存器轮询、批量读写和 PLC 控制。
    /// </summary>
    public sealed class KeyenceKvConnectionPool : IDisposable
    {
        private readonly ConnectionPool<KeyenceKvClient> _pool;
        ...
        public KeyenceKvConnectionPool(
            string ipAddress,
            int port = 3000,
            byte station = 0,
            int timeout = 5000,
            ...
            ILogger? logger = null)
        {
            ...
            _pool = new ConnectionPool<KeyenceKvClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<KeyenceKvClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            KeyenceKvClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"Keyence KV 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<KeyenceKvClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"Keyence KV 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<KeyenceKvClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<KeyenceKvClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        public OperateResult<byte> ReadStatus() => Execute(c => c.ReadStatus());
        ...
        private KeyenceKvClient CreateClient()
        {
            var client = new KeyenceKvClient(_ipAddress, _port, _station, _timeout);
