...
{
    /// <summary>
    /// 基恩士 MC 3E Binary 协议客户端 — 支持 KV-5000/7000 系列 PLC。
    /// 使用 MC 3E Binary 帧格式 (SubHeader=0x5000) + Keyence 专用设备代码。
    /// <para>默认端口: 8501 (Keyence MC 协议标准端口)。</para>
    /// </summary>
    public class KeyenceMcBinaryClient : TcpDeviceBase, IBatchReadWrite, ISubscribeDevice
    {
        ...
        public KeyenceMcBinaryClient(string ip, int port = 8501, int timeout = 5000)
        ...
        private OperateResult<byte[]> ReadWordsBatch(byte subLabel, uint startAddress, ushort count)
        {
        ...
        private OperateResult WriteWordsBatch(byte subLabel, uint startAddress, ushort count, byte[] writeData)
        {
        ...
        private OperateResult<byte[]> ReadWordsRandom((byte subLabel, uint address)[] items)
        {
        ...
        private OperateResult WriteWordsRandom((byte subLabel, uint address, ushort value)[] items)
        {
        ...
        private OperateResult<byte[]> ReadBitsBatch(byte subLabel, uint startAddress, ushort count)
        {
            byte[] data = new byte[6];
            data[0] = subLabel;
            data[1] = (byte)(startAddress & 0xFF);
        ...
        private OperateResult WriteBitsBatch(byte subLabel, uint startAddress, ushort count, byte[] bitData)
        {
        ...
        public OperateResult<Dictionary<string, object?>> BatchRead(IEnumerable<string> addresses)
        {
            ...
            {
                var (subLabel, addrVal) = KeyenceMcAddress.Parse(addr);
        ...
        public OperateResult<Dictionary<string, byte[]>> RandomRead(IEnumerable<string> addresses)
        {
            ...
            {
                var (subLabel, addr) = KeyenceMcAddress.Parse(addrList[i]);
        ...
        public OperateResult BatchWrite(IEnumerable<KeyValuePair<string, object>> items)
        {
            ...
            {
                var (subLabel, addr) = KeyenceMcAddress.Parse(kv.Key);
        ...
        public OperateResult RandomWrite(IEnumerable<KeyValuePair<string, object>> items)
        {
            ...
            {
                var (subLabel, addr) = KeyenceMcAddress.Parse(itemList[i].Key);
