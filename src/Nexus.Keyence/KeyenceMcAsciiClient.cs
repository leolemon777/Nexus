...
{
    /// <summary>
    /// 基恩士 MC 3E ASCII 协议客户端 — 支持 KV-5000/7000 系列 PLC。
    /// 使用 MC 3E ASCII 帧格式 (SubHeader=0x5000, ASCII编码传输) + Keyence 专用设备代码。
    /// <para>默认端口: 8501 (Keyence MC 协议标准端口)。</para>
    /// </summary>
    public class KeyenceMcAsciiClient : TcpDeviceBase, IBatchReadWrite, ISubscribeDevice
    {
        ...
        public KeyenceMcAsciiClient(string ip, int port = 8501, int timeout = 5000)
        ...
        private byte[] BuildAsciiFrame(byte[] binaryFrame)
            => Encoding.ASCII.GetBytes(BitConverter.ToString(binaryFrame).Replace("-", ""));
        ...
        protected override byte[] BuildHeartbeat()
        {
            try
            {
                byte[] frame = BuildMc3EFrame(0x0401, 0x0000, new byte[] { 0xA8, 0x00, 0x00, 0x00, 0x01, 0x00 });
