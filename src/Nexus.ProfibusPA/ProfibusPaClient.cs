...
namespace Nexus.ProfibusPA
{
    public class ProfibusPaClient : SerialDeviceBase, IBatchReadWrite
    {
        private readonly ProfibusDP.ProfibusDpAddressParser _parser = new ProfibusDP.ProfibusDpAddressParser();

        public ProfibusPaClient(ISerialPort port, int timeout = 5000) : base(port, timeout) { }
        protected override int ResponseHeaderLength => 6;
        protected override int GetResponsePayloadLength(byte[] header) { if (header.Length < 6) return 0; return header[4] | (header[5] << 8); }

        private OperateResult<byte[]> ReadData(byte slave, ushort offset, byte length)
        {
            byte[] request = new byte[] { 0x01, slave, 0x0C, 0x00, (byte)(offset & 0xFF), length };
            ...
            if (response.Length < 6) return OperateResult<byte[]>.Failed("Profibus PA 响应过短");
            byte[] data = new byte[response.Length - 6];
            Buffer.BlockCopy(response, 6, data, 0, data.Length);
            return OperateResult<byte[]>.Success(data);
        }

        private OperateResult WriteData(byte slave, ushort offset, byte[] data)
        {
            byte[] request = new byte[6 + data.Length];
            request[0] = 0x01; request[1] = slave; request[2] = 0x0D; request[3] = 0x00;
            request[4] = (byte)(offset & 0xFF); request[5] = (byte)data.Length;
        ...
        public override OperateResult<bool> ReadBool(string address) { var addr = _parser.Parse(address); var r = ReadData(addr.SlaveAddress, addr.Offset, 1); if (!r.IsSuccess) return OperateResult<bool>.Failed(r.Message, r.ErrorCode); return OperateResult<bool>.Success(r.Content.Length > 0 && (r.Content[0] & 0x01) != 0); }
        public override OperateResult<short> ReadInt16(string address) { var addr = _parser.Parse(address); var r = ReadData(addr.SlaveAddress, addr.Offset, 2); if (!r.IsSuccess) return OperateResult<short>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 2) return OperateResult<short>.Failed("数据不足"); return OperateResult<short>.Success((short)((r.Content[0] << 8) | r.Content[1])); }
        ...
        public override OperateResult<string> ReadString(string address, ushort length) { var addr = _parser.Parse(address); var r = ReadData(addr.SlaveAddress, addr.Offset, (byte)length); if (!r.IsSuccess) return OperateResult<string>.Failed(r.Message, r.ErrorCode); return OperateResult<string>.Success(System.Text.Encoding.ASCII.GetString(r.Content, 0, Math.Min(length, r.Content.Length)).TrimEnd('\0')); }
        public override OperateResult<byte[]> ReadBytes(string address, ushort length) { var addr = _parser.Parse(address); return ReadData(addr.SlaveAddress, addr.Offset, (byte)length); }

        public override OperateResult Write(string address, bool value) { var addr = _parser.Parse(address); return WriteData(addr.SlaveAddress, addr.Offset, new byte[] { (byte)(value ? 1 : 0) }); }
        public override OperateResult Write(string address, short value) { var addr = _parser.Parse(address); return WriteData(addr.SlaveAddress, addr.Offset, new byte[] { (byte)(value >> 8), (byte)value }); }
        public override OperateResult Write(string address, ushort value) => Write(address, (short)value);
        public override OperateResult Write(string address, int value) { var addr = _parser.Parse(address); return WriteData(addr.SlaveAddress, addr.Offset, new byte[] { (byte)(value >> 24), (byte)(value >> 16), (byte)(value >> 8), (byte)value }); }
        ...
        public override OperateResult Write(string address, string value) { var addr = _parser.Parse(address); return WriteData(addr.SlaveAddress, addr.Offset, System.Text.Encoding.ASCII.GetBytes(value)); }
        public override OperateResult Write(string address, byte[] data) { var addr = _parser.Parse(address); return WriteData(addr.SlaveAddress, addr.Offset, data); }
