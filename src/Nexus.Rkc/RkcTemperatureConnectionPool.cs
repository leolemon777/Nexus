...
{
    /// <summary>
    /// RKC 温控器连接池 — 复用持久 TCP 连接，降低高频读写的建连成本。
    /// </summary>
    public sealed class RkcTemperatureConnectionPool : IDisposable
    {
        private readonly ConnectionPool<RkcTemperatureClient> _pool;
        ...
        public RkcTemperatureConnectionPool(
            string ip,
            int port = 10001,
            byte station = 1,
            int timeout = 5000,
            ...
            ILogger? logger = null)
        {
            ...
            _key = $"{_ip}:{_port}:{_station}:{_timeout}";
            _pool = new ConnectionPool<RkcTemperatureClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<RkcTemperatureClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            RkcTemperatureClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"RKC 温控器连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<RkcTemperatureClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"RKC 温控器连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<RkcTemperatureClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<RkcTemperatureClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        private RkcTemperatureClient CreateClient()
        {
            var client = new RkcTemperatureClient(_ip, _port, _timeout)
            {
                Station = _station
            };
