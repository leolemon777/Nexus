...
{
    /// <summary>
    /// 三菱 FX5U 客户端 — 基于 MC 协议（二进制格式），支持 FX5U 系列 PLC。
    /// <para>支持设备: D, M, X, Y, T, C, R, SM, SD, W</para>
    /// <para>支持命令: 批量读(0x0401), 批量写(0x1401), 位读(0x0401/子命令1), 位写(0x1401/子命令1)</para>
    /// </summary>
    public class Fx5uClient : TcpDeviceBase, IBatchReadWrite, ISubscribeDevice
    {
        ...
        private readonly Fx5uAddressParser _parser = new Fx5uAddressParser();

        public Fx5uClient(string ip, int port = 4999, int timeout = 5000)
        ...
        private OperateResult<byte[]> SendMc(ushort command, ushort subCommand, byte[] data)
        {
            ...
            if (response.Length < 11) return OperateResult<byte[]>.Failed("FX5U 响应过短");
            ushort completionCode = (ushort)(response[9] | (response[10] << 8));
            if (completionCode != 0)
                return OperateResult<byte[]>.Failed($"FX5U 错误码: 0x{completionCode:X4}", completionCode);
            byte[] pdu = new byte[response.Length - 11];
            Buffer.BlockCopy(response, 11, pdu, 0, pdu.Length);
            return OperateResult<byte[]>.Success(pdu);
        }

        private byte DeviceCode(Fx5uDeviceType type) => type switch
        {
            Fx5uDeviceType.D => 0xA8, Fx5uDeviceType.M => 0x90, Fx5uDeviceType.X => 0x9C,
            Fx5uDeviceType.Y => 0x9D, Fx5uDeviceType.T => 0xC2, Fx5uDeviceType.C => 0xC3,
            Fx5uDeviceType.R => 0xAF, Fx5uDeviceType.SM => 0x91, Fx5uDeviceType.SD => 0xA9,
            Fx5uDeviceType.W => 0xB4, _ => 0xA8
        };

        private byte[] BuildReadData(Fx5uAddress addr, ushort count)
        {
        ...
        private OperateResult WriteWords(Fx5uAddress addr, byte[] wordData)
        {
        ...
        {
            var addr = _parser.Parse(address);
            return WriteWords(addr, new byte[] { (byte)value, (byte)(value >> 8) });
        }
        ...
        {
            var addr = _parser.Parse(address);
            return WriteWords(addr, new byte[] { (byte)value, (byte)(value >> 8), (byte)(value >> 16), (byte)(value >> 24) });
        }
        ...
        {
            var addr = _parser.Parse(address);
            return WriteWords(addr, new byte[] {
        ...
        public override OperateResult Write(string address, string value)
        {
            ...
            return WriteWords(addr, strData);
        }
        ...
        private readonly Dictionary<string, Fx5uSubscription> _subscriptions = new Dictionary<string, Fx5uSubscription>();
        private readonly object _subLock = new object();
        private Timer? _pollTimer;
        private bool _monitoring;

        private class Fx5uSubscription
        {
        ...
        {
            lock (_subLock) { _subscriptions[address] = new Fx5uSubscription { Address = address, DataType = dataType, IntervalMs = intervalMs }; }
        }

        public void Unsubscribe(string address) { lock (_subLock) { _subscriptions.Remove(address); } }

        public void StartSubscriptions(int globalIntervalMs = 500)
        {
            if (_monitoring) return;
            _monitoring = true;
            _pollTimer = new Timer(PollMonitors, null, globalIntervalMs, globalIntervalMs);
        }
        ...
        private void PollMonitors(object? state)
        {
            ...
            try
            {
                List<Fx5uSubscription> entries;
                lock (_subLock) { entries = new List<Fx5uSubscription>(_subscriptions.Values); }
                foreach (var entry in entries)
                {
                    try
                    {
                        ...
                        if (current != null && !Equals(current, entry.LastValue))
                        {
                            ...
                            OnDataChanged?.Invoke(this, new DataChangeEventArgs
                            {
                                Address = entry.Address, OldValue = entry.LastValue, NewValue = current,
                                Timestamp = DateTime.Now, Quality = "Good"
                            });
