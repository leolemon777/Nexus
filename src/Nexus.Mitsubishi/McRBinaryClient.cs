...
{
    /// <summary>
    /// 三菱 MC-R Binary 协议客户端 — 随机读写专用，支持 Q/L/iQ-R 系列。
    /// 基于 SLMP MC 4E Binary 帧格式 (SubHeader=0x5400/0xD400)。
    /// <para>与 MC 3E 的区别: 子头不同，支持扩展随机读写命令。</para>
    /// <para>指令支持: 批量读字(0x0401)、批量写字(0x1401)、随机读取(0x0403)、随机写入(0x1402)。</para>
    /// </summary>
    public class McRBinaryClient : TcpDeviceBase, IBatchReadWrite, ISubscribeDevice
    {
        ...
        /// <summary>多字节数据的字节序 (默认 BigEndian=ABCD)。</summary>
        public Endianness ByteOrder { get; set; } = Endianness.BigEndian;

        /// <summary>字符串编码 (默认 ASCII)。</summary>
        public Encoding StringEncoding { get; set; } = Encoding.ASCII;

        /// <summary>单次批量读取的最大字数 (默认 960)。</summary>
        public ushort MaxReadWordCount { get; set; } = 960;

        /// <summary>单次批量写入的最大字数 (默认 960)。</summary>
        public ushort MaxWriteWordCount { get; set; } = 960;

        public McRBinaryClient(MitsubishiModel model, string ip, int port = 5007, int timeout = 5000)
        ...
        // ── MC 4E 帧结构 ──────────────────────
        // 请求帧:
        //   SubHeader(2) + NetworkNo(1) + PcNo(2) + DestinationStationNo(2)
        //   + WaitTime(2) + Command(2) + SubCommand(2) + Data(变长)
        // 响应帧:
        //   SubHeader(2) + NetworkNo(1) + PcNo(2) + ResponseStationNo(2)
        //   + CompletionCode(2) + Data(变长)
        //
        // SubHeader = 0x54 0x00 (request), 0xD4 0x00 (response)

        protected override int ResponseHeaderLength => 11;
        ...
        protected override OperateResult<byte[]> SendAndReceive(byte[] request)
        {
            try
            {
                ...
                // C1-R2 修复：与基类走同一把 IO 锁（_asyncLock），消除双锁串台。
                AcquireIoLock();
                try
                {
                    ...
                    byte[]? header = ReadExactNs(ns, 11);
                    if (header == null) return OperateResult<byte[]>.Failed("读取 MC-R 响应头失败");

                    ushort completionCode = (ushort)((header[9] << 8) | header[10]);
                    if (completionCode != 0)
                    {
                        string errMsg = completionCode switch
                        {
                            ...
                            0xD003 => "通信对象不存在",
                            0xD004 => "通信对象被其他站占用",
                            _ => $"MC-R 错误码: 0x{completionCode:X4}"
                        };
                    ...
                    byte[] full = new byte[11 + payloadLen];
                    Buffer.BlockCopy(header, 0, full, 0, 11);
                    if (payload != null && payload.Length > 0)
                        Buffer.BlockCopy(payload, 0, full, 11, payload.Length);
            ...
            {
                Log.Error($"MC-R 通讯异常 — {ex.Message}");
                ...
                return OperateResult<byte[]>.Failed($"MC-R 通讯异常: {ex.Message}");
            }
        }
        ...
        private byte[] BuildMcRFrame(ushort command, ushort subCommand, byte[] data)
        {
            int frameLen = 2 + 1 + 2 + 2 + 2 + 2 + 2 + data.Length;
            byte[] frame = new byte[frameLen];
            int offset = 0;

            frame[offset++] = 0x54; frame[offset++] = 0x00;
            frame[offset++] = NetworkNo;
            frame[offset++] = PcNo; frame[offset++] = 0x00;
        ...
        public OperateResult<byte[]> ReadWordsBatch(byte subLabel, uint startAddress, ushort count)
        {
            ...
            var req = BuildMcRFrame(0x0401, 0x0000, data);
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;

            if (resp.Content.Length < 11)
                return OperateResult<byte[]>.Failed("MC-R 响应长度不足");

            int dataLen = resp.Content.Length - 11;
            byte[] result = new byte[dataLen];
            Buffer.BlockCopy(resp.Content, 11, result, 0, dataLen);
        ...
        public OperateResult WriteWordsBatch(byte subLabel, uint startAddress, ushort count, byte[] writeData)
        {
            ...
            var req = BuildMcRFrame(0x1401, 0x0000, data);
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;
        ...
        public OperateResult<byte[]> ReadWordsRandom((byte subLabel, uint address)[] items)
        {
            ...
            var req = BuildMcRFrame(0x0403, 0x0000, data);
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;
        ...
        public OperateResult WriteWordsRandom((byte subLabel, uint address, ushort value)[] items)
        {
            ...
            var req = BuildMcRFrame(0x1402, 0x0000, data);
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;
            return OperateResult.Success();
        }
        ...
        public OperateResult<byte[]> ReadBitsBatch(byte subLabel, uint startAddress, ushort count)
        {
            ...
            var req = BuildMcRFrame(0x0401, 0x0001, data);
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;

            if (resp.Content.Length < 11)
                return OperateResult<byte[]>.Failed("MC-R 位读取响应长度不足");
        ...
        public OperateResult WriteBitsBatch(byte subLabel, uint startAddress, ushort count, byte[] bitData)
        {
            ...
            var req = BuildMcRFrame(0x1401, 0x0001, data);
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;
            return OperateResult.Success();
        }
        ...
        public OperateResult<byte[]> ReadWordsRandomMultiLength((byte subLabel, uint address, ushort length)[] items)
        {
            ...
            for (int i = 0; i < items.Length; i++)
            {
                ...
                data[o + 2] = (byte)((items[i].address >> 8) & 0xFF);
                data[o + 3] = (byte)((items[i].address >> 16) & 0xFF);
                data[o + 4] = (byte)(items[i].length & 0xFF);
                data[o + 5] = (byte)((items[i].length >> 8) & 0xFF);
            }

            var req = BuildMcRFrame(0x0403, 0x0002, data);
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;

            if (resp.Content.Length < 11)
                return OperateResult<byte[]>.Failed("MC-R 多长度随机读取响应长度不足");
        ...
        {
            var req = BuildMcRFrame(0x1001, 0x0000, new byte[] { 0x01, 0x00, 0x00, 0x00 });
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;
            return OperateResult.Success();
        }
        ...
        {
            var req = BuildMcRFrame(0x1002, 0x0000, new byte[] { 0x01, 0x00 });
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;
            return OperateResult.Success();
        }
        ...
        {
            var req = BuildMcRFrame(0x1006, 0x0000, new byte[] { 0x01, 0x00 });
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;
            return OperateResult.Success();
        }
        ...
        {
            var req = BuildMcRFrame(0x0101, 0x0000, Array.Empty<byte>());
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return OperateResult<string>.Failed(resp.Message, resp.ErrorCode);

            if (resp.Content.Length < 11 + 16)
                return OperateResult<string>.Failed("PLC 型号响应数据不足");

            string typeName = Encoding.ASCII.GetString(resp.Content, 11, 16).TrimEnd('\0', ' ');
            return OperateResult<string>.Success(typeName);
        }

        public OperateResult ErrorStateReset()
        {
            var req = BuildMcRFrame(0x1617, 0x0000, Array.Empty<byte>());
            var resp = SendAndReceive(req);
            if (!resp.IsSuccess) return resp;
            return OperateResult.Success();
        }

        public Task<OperateResult> RemoteRunAsync(CancellationToken ct = default) => Task.FromResult(RemoteRun());
        public Task<OperateResult> RemoteStopAsync(CancellationToken ct = default) => Task.FromResult(RemoteStop());
        public Task<OperateResult> RemoteResetAsync(CancellationToken ct = default) => Task.FromResult(RemoteReset());
        public Task<OperateResult<string>> ReadPlcTypeAsync(CancellationToken ct = default) => Task.FromResult(ReadPlcType());
        public Task<OperateResult> ErrorStateResetAsync(CancellationToken ct = default) => Task.FromResult(ErrorStateReset());
        ...
        public OperateResult<Dictionary<string, object?>> BatchRead(IEnumerable<string> addresses)
        {
            ...
            foreach (var kv in groups)
            {
                ...
                {
                    int o = (int)(item.address - minAddr) * 2;
                    if (o + 1 < r.Content.Length)
                    {
                        ushort val = (ushort)((r.Content[o] << 8) | r.Content[o + 1]);
        ...
        public OperateResult<Dictionary<string, byte[]>> RandomRead(IEnumerable<string> addresses)
        {
            ...
            for (int i = 0; i < addrList.Count; i++)
            {
                ...
                if (o + 1 < r.Content.Length)
                {
                    wordData[0] = r.Content[o];
                    wordData[1] = r.Content[o + 1];
                }
