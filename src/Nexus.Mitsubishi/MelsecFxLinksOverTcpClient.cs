...
{
    /// <summary>
    /// 三菱 FX Links 计算机链接协议 over TCP 客户端 — FX 系列 PLC 以太网通信。
    /// <para>适用于 FX 系列 PLC 通过串口服务器/以太网适配器暴露 FX Links 协议的场景。</para>
    ...
    /// </summary>
    public class MelsecFxLinksOverTcpClient : TcpDeviceBase, IBatchReadWrite
    {
        ...
        public MelsecFxLinksOverTcpClient(string ip, int port = DefaultPort, byte station = 0, int timeout = 5000)
        ...
        private OperateResult<string> SendReceiveFxLinks(string cmdData)
        {
            lock (_fxlLock)
            {
                ...
                {
                    return OperateResult<string>.Failed($"FX Links TCP 通讯异常: {ex.Message}");
                }
            }
        }
        ...
        public override string ToString() => $"MelsecFxLinksOverTcpClient[{Ip}:{Port}, Station={Station:D2}]";
    }
}
