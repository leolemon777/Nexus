...
{
    /// <summary>
    /// 三菱 A1E ASCII 协议客户端 — ASCII 编码通讯。
    /// <para>适用于 A 系列 PLC 通过 ASCII-only 串口转 TCP 的场景。</para>
    /// <para>帧结构与 A1E Binary 相同，但所有字节以 ASCII 十六进制编码传输。</para>
    /// <para>地址格式: D100, M100, X0, Y10, S100, B1A, TS100, TC100, TN100, CS100, CC100, CN100, R100, W100, F100。</para>
    /// </summary>
    public class MelsecA1EAsciiClient : TcpDeviceBase, IBatchReadWrite
    {
        private const ushort CodeX  = 0x5820;
        private const ushort CodeY  = 0x5920;
        private const ushort CodeM  = 0x4D20;
        private const ushort CodeS  = 0x5320;
        private const ushort CodeF  = 0x4620;
        private const ushort CodeB  = 0x4220;
        private const ushort CodeTS = 0x5453;
        private const ushort CodeTC = 0x5443;
        private const ushort CodeTN = 0x544E;
        private const ushort CodeCS = 0x4353;
        private const ushort CodeCC = 0x4343;
        private const ushort CodeCN = 0x434E;
        private const ushort CodeD  = 0x4440;
        private const ushort CodeW  = 0x5740;
        private const ushort CodeR  = 0x5220;
        ...
        public MelsecA1EAsciiClient(string ip, int port = 5551, int timeout = 5000)
            : base(ip, port, timeout) { }

        // ── ASCII 编解码 ─────────────────────────

        /// <summary>将二进制帧编码为 ASCII 十六进制帧。</summary>
        private static byte[] ToAsciiHex(byte[] binary)
        {
            char[] hex = new char[binary.Length * 2];
            for (int i = 0; i < binary.Length; i++)
            {
                byte b = binary[i];
                hex[i * 2] = NibbleToHex((byte)(b >> 4));
                hex[i * 2 + 1] = NibbleToHex((byte)(b & 0x0F));
            }
            return Encoding.ASCII.GetBytes(hex);
        }

        /// <summary>将 ASCII 十六进制帧解码为二进制帧。</summary>
        private static byte[] FromAsciiHex(byte[] ascii)
        {
        ...
        private static char NibbleToHex(byte nibble) => (char)(nibble < 10 ? '0' + nibble : 'A' + nibble - 10);
        ...
        public static OperateResult CheckResponse(byte[] response)
        {
            if (response == null || response.Length < 2)
                return OperateResult.Failed($"A1E ASCII 响应过短 ({response?.Length ?? 0} 字节)");
            if (response[1] == 0)
                return OperateResult.Success();
            if (response[1] == 0x5B && response.Length > 2)
                return OperateResult.Failed($"A1E ASCII 错误码: 0x{response[2]:X2}，请参考三菱手册");
            return OperateResult.Failed($"A1E ASCII 错误码: 0x{response[1]:X2}，请参考三菱手册");
        }
        ...
        private static byte[] BuildReadCommandCore(ushort dataCode, int address, int length, bool isBit, byte plcNumber)
        {
            ...
            int encodedLen = length == 256 ? 0 : length;
            var cmd = new byte[12];
            cmd[0] = subCmd;
            cmd[1] = plcNumber;
            cmd[2] = 10;
        ...
        public override string ToString() => $"MelsecA1EAsciiClient[{Ip}:{Port}]";
