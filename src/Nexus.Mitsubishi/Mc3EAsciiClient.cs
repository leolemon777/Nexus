...
namespace Nexus.Mitsubishi
{
    ...
    /// </summary>
    public class Mc3EAsciiClient : TcpDeviceBase, IBatchReadWrite, ISubscribeDevice
    {
        ...
        public Mc3EAsciiClient(MitsubishiModel model, string ip, int port = 5007, int timeout = 5000)
        ...
        // ── MC-3E ASCII 帧结构 ──────────────────
        // 请求帧 (binary): SubHeader(2) + NetworkNo(1) + PcNo(1) + ReqDstStationNo(2) + WaitTime(2)
        //   + Command(2) + SubCommand(2) + Data(变长)
        // 传输时每个 binary byte 编码为 2 个 ASCII hex 字符
        //
        // 响应帧 (解码后 binary):
        //   SubHeader(2) + NetworkNo(1) + PcNo(1) + RespDstStationNo(2) + CompletionCode(2) + Data(变长)
        // CompletionCode at bytes[7-8], 0x0000 = success
        ...
        protected new OperateResult<byte[]> SendAndReceive(byte[] request)
        {
            try
            {
                ...
                using (var ms = new MemoryStream())
                {
                    ...
                    binaryResponse = ParseAsciiResponse(ms.ToArray());
                }
                ...
                if (binaryResponse.Length < 9)
                    return OperateResult<byte[]>.Failed("MC-3E ASCII 响应长度不足");

                ushort completionCode = (ushort)((binaryResponse[7] << 8) | binaryResponse[8]);
                if (completionCode != 0)
                {
        ...
        // ── 批量读字异步 (Command=0x0401, SubCommand=0x0000) ──
        ...
        // ── 批量写字异步 (Command=0x1401, SubCommand=0x0000) ──
        ...
        public OperateResult<byte[]> ReadBitsBatch(byte subLabel, uint startAddress, ushort count)
        {
            ...
            if (resp.Content.Length < 9)
                return OperateResult<byte[]>.Failed("MC-3E ASCII 位读取响应长度不足");
        ...
        public OperateResult<byte[]> ReadWordsRandom((byte subLabel, uint address)[] items)
        {
            ...
            if (resp.Content.Length < 9)
                return OperateResult<byte[]>.Failed("MC-3E ASCII 随机读取响应长度不足");
        ...
        public OperateResult<byte[]> ReadWordsRandomMultiLength((byte subLabel, uint address, ushort length)[] items)
        {
            ...
            if (resp.Content.Length < 9)
                return OperateResult<byte[]>.Failed("MC-3E ASCII 多长度随机读取响应长度不足");
        ...
        /// <summary>异步读取大量数据 — 自动分片。</summary>
        public Task<OperateResult<byte[]>> ReadLargeAsync(string address, ushort length, CancellationToken cancellationToken = default)
            => Task.Run(() => ReadLarge(address, length));

        /// <summary>异步写入大量数据 — 自动分片。</summary>
        public Task<OperateResult> WriteLargeAsync(string address, byte[] data, CancellationToken cancellationToken = default)
            => Task.Run(() => WriteLarge(address, data));
        ...
        public OperateResult<string> ReadPlcType()
        {
            ...
            if (resp.Content.Length < 9)
                return OperateResult<string>.Failed("MC-3E ASCII ReadPlcType 响应长度不足");
