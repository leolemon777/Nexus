...
{
    /// <summary>
    /// KUKA TCP 连接池 — 复用持久 TCP 连接，降低变量读写和程序控制命令的建连成本。
    /// </summary>
    public sealed class KukaTcpConnectionPool : IDisposable
    {
        private readonly ConnectionPool<KukaTcpClient> _pool;
            ...
            ILogger? logger = null)
        {
            ...
            _pool = new ConnectionPool<KukaTcpClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<KukaTcpClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            KukaTcpClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"KUKA TCP 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<KukaTcpClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"KUKA TCP 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<KukaTcpClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<KukaTcpClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        public OperateResult<byte[]> Read(string address) => Execute(c => c.Read(address));
        public OperateResult<string> ReadString(string address) => Execute(c => c.ReadString(address));
        public OperateResult<byte[]> ReadMulti(string[] addresses) => Execute(c => c.ReadMulti(addresses));
        public OperateResult Write(string[] addresses, string[] values) => Execute(c => c.Write(addresses, values));
        public OperateResult StartProgram(string program) => Execute(c => c.StartProgram(program));
        public OperateResult ResetProgram() => Execute(c => c.ResetProgram());
        public OperateResult StopProgram() => Execute(c => c.StopProgram());
        ...
        private KukaTcpClient CreateClient()
        {
            var client = new KukaTcpClient(_ip, _port, _timeout);
