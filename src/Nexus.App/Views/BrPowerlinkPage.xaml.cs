...
public partial class BrPowerlinkPage : Page
{
    private BrPowerlinkViewModel? _vm;

    public BrPowerlinkPage()
    {
        InitializeComponent();
        _vm = ((App)Application.Current).Services.GetRequiredService<BrPowerlinkViewModel>();
        DataContext = _vm;
        Loaded += OnPageLoaded;
