...
{
    /// <summary>
    /// DeviceNet 客户端 — 通过 DeviceNet 网关访问从站对象字典。
    /// <para>地址格式: macId:classId:instanceId:attributeId</para>
    /// </summary>
    public class DeviceNetClient : TcpDeviceBase, IBatchReadWrite
    {
        private readonly DeviceNetAddressParser _parser = new DeviceNetAddressParser();

        public DeviceNetClient(string ip, int port = 5000, int timeout = 5000) : base(ip, port, timeout) { }
        ...
        private OperateResult<byte[]> ReadAttr(byte macId, ushort classId, byte instanceId, byte attributeId)
        {
            byte[] request = new byte[] { 0x01, macId, (byte)(classId >> 8), (byte)(classId & 0xFF), instanceId, attributeId };
            ...
            if (response.Length < 6) return OperateResult<byte[]>.Failed("DeviceNet 响应过短");
            byte[] data = new byte[response.Length - 6];
            Buffer.BlockCopy(response, 6, data, 0, data.Length);
            return OperateResult<byte[]>.Success(data);
        }

        private OperateResult WriteAttr(byte macId, ushort classId, byte instanceId, byte attributeId, byte[] data)
        {
            byte[] request = new byte[6 + data.Length];
            request[0] = 0x02; request[1] = macId;
            request[2] = (byte)(classId >> 8); request[3] = (byte)(classId & 0xFF);
            request[4] = instanceId; request[5] = attributeId;
        ...
        public override OperateResult<bool> ReadBool(string address) { var addr = _parser.Parse(address); var r = ReadAttr(addr.MacId, addr.ClassId, addr.InstanceId, addr.AttributeId); if (!r.IsSuccess) return OperateResult<bool>.Failed(r.Message, r.ErrorCode); return OperateResult<bool>.Success(r.Content.Length > 0 && r.Content[0] != 0); }
        public override OperateResult<short> ReadInt16(string address) { var addr = _parser.Parse(address); var r = ReadAttr(addr.MacId, addr.ClassId, addr.InstanceId, addr.AttributeId); if (!r.IsSuccess) return OperateResult<short>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 2) return OperateResult<short>.Failed("数据不足"); return OperateResult<short>.Success((short)((r.Content[0] << 8) | r.Content[1])); }
        ...
        public override OperateResult<byte[]> ReadBytes(string address, ushort length) { var addr = _parser.Parse(address); return ReadAttr(addr.MacId, addr.ClassId, addr.InstanceId, addr.AttributeId); }

        public override OperateResult Write(string address, bool value) { var addr = _parser.Parse(address); return WriteAttr(addr.MacId, addr.ClassId, addr.InstanceId, addr.AttributeId, new byte[] { (byte)(value ? 1 : 0) }); }
        public override OperateResult Write(string address, short value) { var addr = _parser.Parse(address); return WriteAttr(addr.MacId, addr.ClassId, addr.InstanceId, addr.AttributeId, new byte[] { (byte)(value >> 8), (byte)value }); }
        public override OperateResult Write(string address, ushort value) => Write(address, (short)value);
        public override OperateResult Write(string address, int value) { var addr = _parser.Parse(address); return WriteAttr(addr.MacId, addr.ClassId, addr.InstanceId, addr.AttributeId, new byte[] { (byte)(value >> 24), (byte)(value >> 16), (byte)(value >> 8), (byte)value }); }
        ...
        public override OperateResult Write(string address, string value) { var addr = _parser.Parse(address); return WriteAttr(addr.MacId, addr.ClassId, addr.InstanceId, addr.AttributeId, System.Text.Encoding.ASCII.GetBytes(value)); }
        public override OperateResult Write(string address, byte[] data) { var addr = _parser.Parse(address); return WriteAttr(addr.MacId, addr.ClassId, addr.InstanceId, addr.AttributeId, data); }
