...
{
    public class VigorSerialClient : SerialDeviceBase, IBatchReadWrite
    {
        ...
        public VigorSerialClient(ISerialPort serialPort, byte station = 1, int timeout = 3000)
        ...
        public override OperateResult<bool> ReadBool(string address)
        {
            try
            {
                ...
                var resp = VigorSendAndReceive(cmd);
        ...
        private OperateResult<byte[]> VigorSendAndReceive(byte[] frame)
        {
            lock (Port)
            {
                try
                {
                    if (!Port.IsOpen)
                        return OperateResult<byte[]>.Failed("Serial port is not open");
                    ...
                    var response = ReadVigorResponse();
                    if (response == null)
                ...
                {
                    RaiseError($"Vigor serial communication error: {ex.Message}");
                    return OperateResult<byte[]>.Failed($"Vigor serial communication error: {ex.Message}");
                }
            }
        }
        ...
        private byte[]? ReadVigorResponse()
        {
            ...
            while (Environment.TickCount < deadline)
            {
                ...
                {
                    int next = ReadOneByte(singleBuf);
                    if (next < 0) return null;
                    if ((byte)next == VigorConstants.ETX)
                    {
                        int bcc1 = ReadOneByte(singleBuf);
                        int bcc2 = ReadOneByte(singleBuf);
        ...
        private int ReadOneByte(byte[] buf)
        {
            try
            {
        ...
        public override string ToString() => $"VigorSerialClient[Station={Station}]";
    }
}
