...
{
    /// <summary>
    /// LonWorks 客户端 — 通过 LonWorks 网关访问网络变量。
    /// <para>地址格式: node:nvIndex 或 nvIndex</para>
    /// </summary>
    public class LonWorksClient : TcpDeviceBase, IBatchReadWrite
    {
        public byte DomainId { get; set; } = 0;
        private readonly LonWorksAddressParser _parser = new LonWorksAddressParser();

        public LonWorksClient(string ip, int port = 1628, int timeout = 5000) : base(ip, port, timeout) { }
        ...
        private OperateResult<byte[]> ReadNv(ushort nodeId, ushort nvIndex, byte length)
        {
            byte[] request = new byte[] { 0x01, (byte)(nodeId >> 8), (byte)nodeId, (byte)(nvIndex >> 8), (byte)nvIndex, length };
            ...
            if (response.Length < 6) return OperateResult<byte[]>.Failed("LonWorks 响应过短");
            byte[] data = new byte[response.Length - 6];
            Buffer.BlockCopy(response, 6, data, 0, data.Length);
            return OperateResult<byte[]>.Success(data);
        }

        private OperateResult WriteNv(ushort nodeId, ushort nvIndex, byte[] data)
        {
            byte[] request = new byte[6 + data.Length];
            request[0] = 0x02;
            request[1] = (byte)(nodeId >> 8); request[2] = (byte)nodeId;
            request[3] = (byte)(nvIndex >> 8); request[4] = (byte)nvIndex;
        ...
        public override OperateResult<bool> ReadBool(string address) { var addr = _parser.Parse(address); var r = ReadNv(addr.NodeId, addr.NetworkVariableIndex, 1); if (!r.IsSuccess) return OperateResult<bool>.Failed(r.Message, r.ErrorCode); return OperateResult<bool>.Success(r.Content.Length > 0 && r.Content[0] != 0); }
        public override OperateResult<short> ReadInt16(string address) { var addr = _parser.Parse(address); var r = ReadNv(addr.NodeId, addr.NetworkVariableIndex, 2); if (!r.IsSuccess) return OperateResult<short>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 2) return OperateResult<short>.Failed("数据不足"); return OperateResult<short>.Success((short)((r.Content[0] << 8) | r.Content[1])); }
        ...
        public override OperateResult<string> ReadString(string address, ushort length) { var addr = _parser.Parse(address); var r = ReadNv(addr.NodeId, addr.NetworkVariableIndex, (byte)length); if (!r.IsSuccess) return OperateResult<string>.Failed(r.Message, r.ErrorCode); return OperateResult<string>.Success(System.Text.Encoding.ASCII.GetString(r.Content, 0, Math.Min(length, r.Content.Length)).TrimEnd('\0')); }
        public override OperateResult<byte[]> ReadBytes(string address, ushort length) { var addr = _parser.Parse(address); return ReadNv(addr.NodeId, addr.NetworkVariableIndex, (byte)length); }

        public override OperateResult Write(string address, bool value) { var addr = _parser.Parse(address); return WriteNv(addr.NodeId, addr.NetworkVariableIndex, new byte[] { (byte)(value ? 1 : 0) }); }
        public override OperateResult Write(string address, short value) { var addr = _parser.Parse(address); return WriteNv(addr.NodeId, addr.NetworkVariableIndex, new byte[] { (byte)(value >> 8), (byte)value }); }
        public override OperateResult Write(string address, ushort value) => Write(address, (short)value);
        public override OperateResult Write(string address, int value) { var addr = _parser.Parse(address); return WriteNv(addr.NodeId, addr.NetworkVariableIndex, new byte[] { (byte)(value >> 24), (byte)(value >> 16), (byte)(value >> 8), (byte)value }); }
        ...
        public override OperateResult Write(string address, string value) { var addr = _parser.Parse(address); return WriteNv(addr.NodeId, addr.NetworkVariableIndex, System.Text.Encoding.ASCII.GetBytes(value)); }
        public override OperateResult Write(string address, byte[] data) { var addr = _parser.Parse(address); return WriteNv(addr.NodeId, addr.NetworkVariableIndex, data); }
