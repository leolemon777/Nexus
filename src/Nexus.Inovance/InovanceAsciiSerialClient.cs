...
{
    /// <summary>
    /// 汇川 Modbus ASCII 串口客户端。
    /// 通过 Modbus ASCII 串口协议访问汇川 AM/H3U/H5U 系列 PLC。
    /// 默认字节序为 MidLittleEndian (CDAB)。
    /// </summary>
    public class InovanceAsciiSerialClient : ModbusAsciiClient
    {
        /// <summary>汇川 PLC 系列。</summary>
        public InovanceSeries Series { get; set; }

        /// <summary>
        /// 创建汇川 Modbus ASCII 串口客户端实例。
        /// </summary>
        ...
        public InovanceAsciiSerialClient(ISerialPort serialPort, byte station = 1, int timeout = 5000, InovanceSeries series = InovanceSeries.AM)
        ...
        {
            return $"InovanceAsciiSerial[{Series}]";
        }
    }
}
