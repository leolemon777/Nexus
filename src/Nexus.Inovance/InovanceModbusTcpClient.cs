...
namespace Nexus.Inovance
{
    /// <summary>
    /// 汇川 Modbus TCP 客户端。
    /// 通过标准 Modbus TCP 协议访问汇川 AM/H3U/H5U 系列 PLC。
    ...
    public class InovanceModbusTcpClient : ModbusTcpClient
    {
        ...
        public InovanceModbusTcpClient(string ip, int port = 502, byte station = 1, int timeout = 5000, InovanceSeries series = InovanceSeries.AM)
        ...
        {
            return $"InovanceModbusTcp[{Series}][{Ip}:{Port}]";
        }
    }
}
