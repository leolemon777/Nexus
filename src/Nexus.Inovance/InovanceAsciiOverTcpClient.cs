...
{
    /// <summary>
    /// 汇川 Modbus ASCII Over TCP 客户端。
    /// 通过 TCP 传输 Modbus ASCII 格式报文访问汇川 AM/H3U/H5U 系列 PLC。
    /// 默认字节序为 MidLittleEndian (CDAB)。
    /// </summary>
    public class InovanceAsciiOverTcpClient : ModbusTcpClient
    {
        /// <summary>汇川 PLC 系列。</summary>
        public InovanceSeries Series { get; set; }

        /// <summary>
        /// 创建汇川 Modbus ASCII Over TCP 客户端实例。
        /// </summary>
        ...
        public InovanceAsciiOverTcpClient(string ip, int port = 502, byte station = 1, int timeout = 5000, InovanceSeries series = InovanceSeries.AM)
        ...
        {
            return $"InovanceAsciiOverTcp[{Series}][{Ip}:{Port}]";
        }
    }
}
