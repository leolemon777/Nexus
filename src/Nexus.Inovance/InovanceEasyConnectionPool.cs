...
{
    /// <summary>
    /// Inovance EasyNet 连接池 — 复用持久 TCP 连接，适合高频寄存器轮询和批量读写。
    /// </summary>
    public sealed class InovanceEasyConnectionPool : IDisposable
    {
        private readonly ConnectionPool<InovanceEasyClient> _pool;
        ...
        public InovanceEasyConnectionPool(
            string ip,
            int port = 502,
            int timeout = 5000,
            int maxPoolSize = 5,
            ...
            ILogger? logger = null)
        {
            ...
            _pool = new ConnectionPool<InovanceEasyClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<InovanceEasyClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            InovanceEasyClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"Inovance EasyNet 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<InovanceEasyClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"Inovance EasyNet 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<InovanceEasyClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<InovanceEasyClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        private InovanceEasyClient CreateClient()
        {
            var client = new InovanceEasyClient(_ip, _port, _timeout);
