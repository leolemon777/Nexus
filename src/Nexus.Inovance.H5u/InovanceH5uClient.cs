...
{
    /// <summary>
    /// 汇川 H5U 客户端 — 基于 Modbus TCP，支持汇川专用地址格式。
    /// <para>支持地址: D(数据寄存器), M(位), X(输入), Y(输出), T(定时器), C(计数器), S(步)</para>
    /// <para>支持功能码: FC01-06, FC15, FC16</para>
    /// </summary>
    public class InovanceH5uClient : TcpDeviceBase, IBatchReadWrite, ISubscribeDevice
    {
        public byte Station { get; set; }
        public Endianness ByteOrder { get; set; } = Endianness.BigEndian;

        private readonly InovanceH5uAddressParser _parser = new InovanceH5uAddressParser();
        private int _transactionId;

        public InovanceH5uClient(string ip, int port = 502, byte station = 1, int timeout = 5000)
        ...
        {
            var addr = _parser.Parse(address);
            var r = SendModbus(new byte[] { addr.WriteFunctionCode, (byte)(addr.StartAddress >> 8), (byte)addr.StartAddress, (byte)(value ? 0xFF : 0x00), 0x00 });
        ...
        private readonly Dictionary<string, SubscriptionEntry> _subscriptions = new Dictionary<string, SubscriptionEntry>();
        ...
        {
            lock (_subLock) { _subscriptions[address] = new SubscriptionEntry { Address = address, DataType = dataType, IntervalMs = intervalMs }; }
        }

        public void Unsubscribe(string address)
        {
            lock (_subLock) { _subscriptions.Remove(address); }
        }
        ...
        private void PollMonitors(object? state)
        {
            ...
            try
            {
                ...
                lock (_subLock) { entries = new List<SubscriptionEntry>(_subscriptions.Values); }
