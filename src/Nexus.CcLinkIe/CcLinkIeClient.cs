...
{
    /// <summary>
    /// CC-Link IE 客户端 — 通过 TCP 连接三菱 CC-Link IE 控制器。
    /// <para>支持设备: R, WR, LR, SW, SB, DX, DY, W, B, D</para>
    /// <para>基于 MC 协议（二进制格式）实现。</para>
    /// </summary>
    public class CcLinkIeClient : TcpDeviceBase, IBatchReadWrite
    {
        private readonly CcLinkIeAddressParser _parser = new CcLinkIeAddressParser();

        public CcLinkIeClient(string ip, int port = 4999, int timeout = 5000)
            : base(ip, port, timeout) { }

        protected override int ResponseHeaderLength => 9;
        protected override int GetResponsePayloadLength(byte[] header) => 0;

        private byte DeviceCode(CcLinkIeDeviceType type) => type switch
        {
            CcLinkIeDeviceType.R => 0xAF, CcLinkIeDeviceType.WR => 0xB4, CcLinkIeDeviceType.LR => 0xA0,
            CcLinkIeDeviceType.SW => 0xB5, CcLinkIeDeviceType.SB => 0xA1, CcLinkIeDeviceType.DX => 0x9C,
            CcLinkIeDeviceType.DY => 0x9D, CcLinkIeDeviceType.W => 0xB4, CcLinkIeDeviceType.B => 0xA0,
            CcLinkIeDeviceType.D => 0xA8, _ => 0xA8
        };
        ...
        private OperateResult<byte[]> SendMc(ushort command, ushort subCommand, byte[] data)
        {
            ...
            if (response.Length < 11) return OperateResult<byte[]>.Failed("CC-Link IE 响应过短");
            ushort completionCode = (ushort)(response[9] | (response[10] << 8));
            if (completionCode != 0) return OperateResult<byte[]>.Failed($"CC-Link IE 错误: 0x{completionCode:X4}", completionCode);
        ...
        public override OperateResult<bool> ReadBool(string address)
        {
            ...
            byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, DeviceCode(addr.DeviceType), 1, 0 };
        ...
        {
            var addr = _parser.Parse(address);
            byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, DeviceCode(addr.DeviceType), 2, 0 };
            var r = SendMc(0x0401, 0x0000, data);
        ...
        {
            var addr = _parser.Parse(address);
            byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, DeviceCode(addr.DeviceType), 4, 0 };
            var r = SendMc(0x0401, 0x0000, data);
        ...
        public override OperateResult<string> ReadString(string address, ushort length)
        {
            ...
            byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, DeviceCode(addr.DeviceType), (byte)(regCount & 0xFF), (byte)(regCount >> 8) };
            var r = SendMc(0x0401, 0x0000, data);
        ...
        private OperateResult WriteWords(CcLinkIeAddress addr, byte[] wordData)
        {
            ...
            data[2] = 0; data[3] = DeviceCode(addr.DeviceType);
        ...
        {
            var addr = _parser.Parse(address);
            byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, DeviceCode(addr.DeviceType), 1, 0, (byte)(value ? 0x01 : 0x00) };
