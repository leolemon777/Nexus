...
{
    /// <summary>
    /// AS-i (Actuator-Sensor Interface) 客户端 — 通过 AS-i 网关访问从站 I/O。
    /// <para>地址格式: I{slave}[.offset] 或 Q{slave}[.offset]</para>
    /// </summary>
    public class ASiClient : TcpDeviceBase, IBatchReadWrite
    {
        private readonly ASiAddressParser _parser = new ASiAddressParser();

        public ASiClient(string ip, int port = 5000, int timeout = 5000) : base(ip, port, timeout) { }
        ...
        private OperateResult<byte[]> ReadIo(byte slave, byte offset, byte length, bool isInput)
        {
            byte[] request = new byte[] { (byte)(isInput ? 0x01 : 0x02), slave, offset, length };
            ...
            if (response.Length < 4) return OperateResult<byte[]>.Failed("AS-i 响应过短");
            byte[] data = new byte[response.Length - 4];
            Buffer.BlockCopy(response, 4, data, 0, data.Length);
            return OperateResult<byte[]>.Success(data);
        }

        private OperateResult WriteIo(byte slave, byte offset, byte[] data, bool isInput)
        {
            byte[] request = new byte[4 + data.Length];
            request[0] = (byte)(isInput ? 0x03 : 0x04);
            request[1] = slave; request[2] = offset; request[3] = (byte)data.Length;
        ...
        public override OperateResult<bool> ReadBool(string address) { var addr = _parser.Parse(address); var r = ReadIo(addr.SlaveAddress, addr.Offset, 1, addr.IsInput); if (!r.IsSuccess) return OperateResult<bool>.Failed(r.Message, r.ErrorCode); return OperateResult<bool>.Success(r.Content.Length > 0 && r.Content[0] != 0); }
        public override OperateResult<short> ReadInt16(string address) { var addr = _parser.Parse(address); var r = ReadIo(addr.SlaveAddress, addr.Offset, 2, addr.IsInput); if (!r.IsSuccess) return OperateResult<short>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 2) return OperateResult<short>.Failed("数据不足"); return OperateResult<short>.Success((short)((r.Content[0] << 8) | r.Content[1])); }
        ...
        public override OperateResult<string> ReadString(string address, ushort length) { var addr = _parser.Parse(address); var r = ReadIo(addr.SlaveAddress, addr.Offset, (byte)length, addr.IsInput); if (!r.IsSuccess) return OperateResult<string>.Failed(r.Message, r.ErrorCode); return OperateResult<string>.Success(System.Text.Encoding.ASCII.GetString(r.Content, 0, Math.Min(length, r.Content.Length)).TrimEnd('\0')); }
        public override OperateResult<byte[]> ReadBytes(string address, ushort length) { var addr = _parser.Parse(address); return ReadIo(addr.SlaveAddress, addr.Offset, (byte)length, addr.IsInput); }

        public override OperateResult Write(string address, bool value) { var addr = _parser.Parse(address); return WriteIo(addr.SlaveAddress, addr.Offset, new byte[] { (byte)(value ? 1 : 0) }, addr.IsInput); }
        public override OperateResult Write(string address, short value) { var addr = _parser.Parse(address); return WriteIo(addr.SlaveAddress, addr.Offset, new byte[] { (byte)(value >> 8), (byte)value }, addr.IsInput); }
        public override OperateResult Write(string address, ushort value) => Write(address, (short)value);
        public override OperateResult Write(string address, int value) { var addr = _parser.Parse(address); return WriteIo(addr.SlaveAddress, addr.Offset, new byte[] { (byte)(value >> 24), (byte)(value >> 16), (byte)(value >> 8), (byte)value }, addr.IsInput); }
        ...
        public override OperateResult Write(string address, string value) { var addr = _parser.Parse(address); return WriteIo(addr.SlaveAddress, addr.Offset, System.Text.Encoding.ASCII.GetBytes(value), addr.IsInput); }
        public override OperateResult Write(string address, byte[] data) { var addr = _parser.Parse(address); return WriteIo(addr.SlaveAddress, addr.Offset, data, addr.IsInput); }
