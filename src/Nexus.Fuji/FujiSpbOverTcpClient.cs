...
{
    /// <summary>
    /// 富士 SPB 系列 PLC TCP 通讯客户端 — SPB 协议 Over TCP。
    /// <para>通过 TCP 连接发送 SPB ASCII 帧，适用于以太网串口服务器透传场景。</para>
    /// <para>帧格式与串口版相同：':' + Station(2hex) + ByteCount(2hex) + Cmd + Data + LRC(2hex) + CR+LF</para>
    /// </summary>
    public class FujiSpbOverTcpClient : TcpDeviceBase, IBatchReadWrite
    {
        ...
        public FujiSpbOverTcpClient(string ip, int port = 5000, byte station = 1, int timeout = 5000)
        ...
        private OperateResult<string> SendReceiveAscii(string frame)
        {
            try
            {
                ...
                if (!connResult.IsSuccess)
                    return OperateResult<string>.Failed(connResult.Message);

                var sendResult = SendAndReceive(Encoding.ASCII.GetBytes(frame));
                if (!sendResult.IsSuccess)
                    return OperateResult<string>.Failed(sendResult.Message);

                string response = Encoding.ASCII.GetString(sendResult.Content);
                RaiseMessageReceived(response.Replace("\r", "\\r").Replace("\n", "\\n"));

                int lfIdx = response.IndexOf('\n');
                if (lfIdx < 0)
                    return OperateResult<string>.Failed("SPB 响应缺少换行符");

                return OperateResult<string>.Success(response.Substring(0, lfIdx + 1));
            }
            catch (Exception ex)
            {
                RaiseError($"SPB TCP 通讯异常: {ex.Message}");
                return OperateResult<string>.Failed($"SPB TCP 通讯异常: {ex.Message}");
            }
        }

        private OperateResult EnsureConnected()
        {
            if (IsConnected) return OperateResult.Success();
            return Connect();
        }
