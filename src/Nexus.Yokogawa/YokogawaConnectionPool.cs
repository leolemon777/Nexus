...
{
    /// <summary>
    /// Yokogawa 二进制链接协议连接池 — 复用持久连接，降低高频读写和批量操作的建连成本。
    /// </summary>
    public sealed class YokogawaConnectionPool : IDisposable
    {
        private readonly ConnectionPool<YokogawaClient> _pool;
        private readonly string _key;
        private readonly string _ip;
        private readonly int _port;
        private readonly int _timeout;
        private readonly byte _cpuNumber;
            ...
            ILogger? logger = null)
        {
            ...
            _key = $"{_ip}:{_port}:{_timeout}:{_cpuNumber}";
            _pool = new ConnectionPool<YokogawaClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<YokogawaClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            YokogawaClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"Yokogawa 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<YokogawaClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"Yokogawa 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<YokogawaClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<YokogawaClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        public OperateResult Start() => Execute(c => c.Start());
        public OperateResult Stop() => Execute(c => c.Stop());

        public OperateResult<byte[]> ReadRandomWords(string[] addresses) => Execute(c => c.ReadRandomWords(addresses));
        public OperateResult WriteRandomWords(string[] addresses, byte[][] data) => Execute(c => c.WriteRandomWords(addresses, data));
        public OperateResult<short[]> ReadRandomInt16(string[] addresses) => Execute(c => c.ReadRandomInt16(addresses));
        public OperateResult<ushort[]> ReadRandomUInt16(string[] addresses) => Execute(c => c.ReadRandomUInt16(addresses));
        ...
        private YokogawaClient CreateClient()
        {
            var client = new YokogawaClient(_ip, _port, _timeout)
            {
                CpuNumber = _cpuNumber
            };
