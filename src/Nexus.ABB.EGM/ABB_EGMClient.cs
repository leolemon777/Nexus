...
namespace Nexus.ABB.EGM
{
    public sealed class ABB_EGMAddress : IDataAddress
    {
        public string Original { get; }
        public string RobTarget { get; }
        public ABB_EGMAddress(string original, string robTarget) { Original = original; RobTarget = robTarget; }
    }

    public sealed class ABB_EGMAddressParser : IAddressParser<ABB_EGMAddress>
    {
        public ABB_EGMAddress Parse(string address)
        {
            if (string.IsNullOrWhiteSpace(address)) throw new AddressParseException(address, "地址不能为空");
            return new ABB_EGMAddress(address, address.Trim());
        }
        public bool TryParse(string address, out ABB_EGMAddress? parsed) { try { parsed = Parse(address); return true; } catch { parsed = null; return false; } }
    }

    /// <summary>
    /// ABB EGM (Externally Guided Motion) 客户端 — 实时运动控制接口。
    /// <para>地址格式: robTarget 名称 (如 robtarget1)</para>
    /// </summary>
    public class ABB_EGMClient : TcpDeviceBase, IBatchReadWrite
    {
        private readonly ABB_EGMAddressParser _parser = new ABB_EGMAddressParser();

        public ABB_EGMClient(string ip, int port = 6510, int timeout = 5000) : base(ip, port, timeout) { }
        ...
        private OperateResult<byte[]> ReadEGM(string target)
        {
            byte[] nameBytes = System.Text.Encoding.ASCII.GetBytes(target);
            byte[] request = new byte[4 + nameBytes.Length];
            request[0] = 0x01; request[1] = 0x00;
            request[2] = (byte)(nameBytes.Length >> 8); request[3] = (byte)nameBytes.Length;
            Buffer.BlockCopy(nameBytes, 0, request, 4, nameBytes.Length);
            ...
            if (response.Length < 4) return OperateResult<byte[]>.Failed("EGM 响应过短");
            byte[] data = new byte[response.Length - 4];
            Buffer.BlockCopy(response, 4, data, 0, data.Length);
            return OperateResult<byte[]>.Success(data);
        }

        private OperateResult WriteEGM(string target, byte[] data)
        {
            byte[] nameBytes = System.Text.Encoding.ASCII.GetBytes(target);
            byte[] request = new byte[4 + nameBytes.Length + data.Length];
            request[0] = 0x02; request[1] = 0x00;
            request[2] = (byte)(nameBytes.Length >> 8); request[3] = (byte)nameBytes.Length;
            Buffer.BlockCopy(nameBytes, 0, request, 4, nameBytes.Length);
            Buffer.BlockCopy(data, 0, request, 4 + nameBytes.Length, data.Length);
            var result = SendAndReceive(request);
            return result.IsSuccess ? OperateResult.Success() : OperateResult.Failed(result.Message, result.ErrorCode);
        }

        public override OperateResult<bool> ReadBool(string address) { var r = ReadEGM(address); if (!r.IsSuccess) return OperateResult<bool>.Failed(r.Message, r.ErrorCode); return OperateResult<bool>.Success(r.Content.Length > 0 && r.Content[0] != 0); }
        public override OperateResult<short> ReadInt16(string address) { var r = ReadEGM(address); if (!r.IsSuccess) return OperateResult<short>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 2) return OperateResult<short>.Failed("数据不足"); return OperateResult<short>.Success((short)(r.Content[0] | (r.Content[1] << 8))); }
        ...
        public override OperateResult<int> ReadInt32(string address) { var r = ReadEGM(address); if (!r.IsSuccess) return OperateResult<int>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 4) return OperateResult<int>.Failed("数据不足"); return OperateResult<int>.Success(r.Content[0] | (r.Content[1] << 8) | (r.Content[2] << 16) | (r.Content[3] << 24)); }
        ...
        public override OperateResult<float> ReadFloat(string address) { var r = ReadEGM(address); if (!r.IsSuccess) return OperateResult<float>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 4) return OperateResult<float>.Failed("数据不足"); return OperateResult<float>.Success(DataConverter.ToFloat(r.Content, 0)); }
        ...
        public override OperateResult<string> ReadString(string address, ushort length) { var r = ReadEGM(address); if (!r.IsSuccess) return OperateResult<string>.Failed(r.Message, r.ErrorCode); return OperateResult<string>.Success(System.Text.Encoding.ASCII.GetString(r.Content, 0, Math.Min(length, r.Content.Length)).TrimEnd('\0')); }
        public override OperateResult<byte[]> ReadBytes(string address, ushort length) { return ReadEGM(address); }

        public override OperateResult Write(string address, bool value) { return WriteEGM(address, new byte[] { (byte)(value ? 1 : 0) }); }
        public override OperateResult Write(string address, short value) { return WriteEGM(address, new byte[] { (byte)value, (byte)(value >> 8) }); }
        public override OperateResult Write(string address, ushort value) => Write(address, (short)value);
        public override OperateResult Write(string address, int value) { return WriteEGM(address, new byte[] { (byte)value, (byte)(value >> 8), (byte)(value >> 16), (byte)(value >> 24) }); }
        ...
        public override OperateResult Write(string address, string value) { return WriteEGM(address, System.Text.Encoding.ASCII.GetBytes(value)); }
        public override OperateResult Write(string address, byte[] data) { return WriteEGM(address, data); }
