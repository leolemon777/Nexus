...
{
    /// <summary>
    /// Toledo 电子秤连接池 — 复用持久 TCP 连接，降低连续读取称重帧的建连成本。
    /// </summary>
    public sealed class ToledoConnectionPool : IDisposable
    {
        private readonly ConnectionPool<ToledoClient> _pool;
            ...
            ILogger? logger = null)
        {
            ...
            _pool = new ConnectionPool<ToledoClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<ToledoClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            ToledoClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"Toledo 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<ToledoClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"Toledo 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<ToledoClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<ToledoClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        public OperateResult<ToledoStandardData> ReadWeight() => Execute(c => c.ReadWeight());
        public OperateResult<byte[]> ReadRaw() => Execute(c => c.ReadRaw());

        public Task<OperateResult<ToledoStandardData>> ReadWeightAsync(CancellationToken cancellationToken = default)
            => ExecuteAsync(c => Task.FromResult(c.ReadWeight()), cancellationToken);

        public Task<OperateResult<byte[]>> ReadRawAsync(CancellationToken cancellationToken = default)
            => ExecuteAsync(c => Task.FromResult(c.ReadRaw()), cancellationToken);
        ...
        private ToledoClient CreateClient()
        {
            var client = new ToledoClient(_ip, _port, _timeout);
