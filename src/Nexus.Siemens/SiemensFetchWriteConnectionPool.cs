...
{
    /// <summary>
    /// Siemens Fetch/Write 连接池 — 复用持久 TCP 连接，减少短连接频繁建立的开销。
    /// </summary>
    public sealed class SiemensFetchWriteConnectionPool : IDisposable
    {
        private readonly ConnectionPool<SiemensFetchWriteClient> _pool;
        ...
        public SiemensFetchWriteConnectionPool(
            string ip,
            int port = 102,
            int timeout = 5000,
            int maxPoolSize = 5,
            ...
            ILogger? logger = null)
        {
            ...
            _pool = new ConnectionPool<SiemensFetchWriteClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<SiemensFetchWriteClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            SiemensFetchWriteClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"Siemens Fetch/Write 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<SiemensFetchWriteClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"Siemens Fetch/Write 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<SiemensFetchWriteClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<SiemensFetchWriteClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        private SiemensFetchWriteClient CreateClient()
        {
            var client = new SiemensFetchWriteClient(_ip, _port, _timeout);
