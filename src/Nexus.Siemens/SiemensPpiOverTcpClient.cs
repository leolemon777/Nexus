...
{
    /// <summary>
    /// 西门子 PPI over TCP 客户端。
    /// <para>通过串口转 TCP 网关（如 USR-TCP232）透明转发 PPI 串口帧到 TCP 连接。</para>
    /// <para>帧格式与 SiemensPpiClient 完全一致，仅传输层从串口改为 TCP。</para>
    /// <para>地址格式：V100, V100.0, I100, Q100, M100, S100, SM100, C100。</para>
    /// </summary>
    public class SiemensPpiOverTcpClient : TcpDeviceBase, IBatchReadWrite, ISubscribeDevice
    {
        public byte MasterAddress { get; set; } = 1;
        public byte SlaveAddress { get; set; } = 2;

        public SiemensPpiOverTcpClient(string ip, int port = 502, int timeout = 5000)
        ...
        {
            if (header == null || header.Length < 8 || header[0] != 0x68 || header[3] != 0x68) return 0;
            int len = header[1];
            int remaining = len - 2;
        ...
        private OperateResult<byte[]> SendPpi(byte control, byte functionCode, byte[] data)
        {
        ...
        public override string ToString() => $"SiemensPpiOverTcpClient[{Ip}:{Port}]";
    }
}
