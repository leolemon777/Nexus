...
{
    /// <summary>
    /// IEC 61850 MMS 连接池 — 复用持久 TCP 会话，降低 IED 高频轮询和控制写入的建连成本。
    /// </summary>
    public sealed class Iec61850ConnectionPool : IDisposable
    {
        private readonly ConnectionPool<Iec61850Client> _pool;
        private readonly string _key;
        private readonly string _ip;
        private readonly int _port;
        private readonly int _timeout;
        private readonly string _logicalDevice;
        private readonly ReportTriggerOptions _triggerOptions;
        ...
        public Iec61850ConnectionPool(
            string ip,
            int port = Iec61850Constants.DefaultMmsPort,
            string logicalDevice = "LD0",
            ReportTriggerOptions triggerOptions = ReportTriggerOptions.DataChanged | ReportTriggerOptions.QualityChanged,
            int timeout = 5000,
            int maxPoolSize = 5,
            ...
            ILogger? logger = null)
        {
            ...
            _port = port;
            _timeout = timeout;
            _logicalDevice = logicalDevice ?? throw new ArgumentNullException(nameof(logicalDevice));
            _triggerOptions = triggerOptions;
            _logger = logger ?? NullLogger.Instance;
            _key = $"{_ip}:{_port}:{_logicalDevice}:{(ushort)_triggerOptions}:{_timeout}";
            _pool = new ConnectionPool<Iec61850Client>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<Iec61850Client, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            Iec61850Client? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"IEC 61850 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<Iec61850Client, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"IEC 61850 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<Iec61850Client, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<Iec61850Client, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        private Iec61850Client CreateClient()
        {
            var client = new Iec61850Client(_ip, _port, _timeout)
            {
