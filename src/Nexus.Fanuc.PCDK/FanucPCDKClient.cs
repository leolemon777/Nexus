...
namespace Nexus.Fanuc.PCDK
{
    public sealed class FanucPCDKAddress : IDataAddress
    {
        public string Original { get; }
        public string VarName { get; }
        public FanucPCDKAddress(string original, string varName) { Original = original; VarName = varName; }
    }

    public sealed class FanucPCDKAddressParser : IAddressParser<FanucPCDKAddress>
    {
        public FanucPCDKAddress Parse(string address)
        {
            if (string.IsNullOrWhiteSpace(address)) throw new AddressParseException(address, "地址不能为空");
            return new FanucPCDKAddress(address, address.Trim());
        }
        public bool TryParse(string address, out FanucPCDKAddress? parsed) { try { parsed = Parse(address); return true; } catch { parsed = null; return false; } }
    }

    /// <summary>
    /// FANUC PCDK 客户端 — 通过 FANUC PCDK 接口读写机器人变量。
    /// <para>地址格式: 变量名 (如 R[1], DI[1], DO[1], PR[1])</para>
    /// </summary>
    public class FanucPCDKClient : TcpDeviceBase, IBatchReadWrite
    {
        private readonly FanucPCDKAddressParser _parser = new FanucPCDKAddressParser();

        public FanucPCDKClient(string ip, int port = 60008, int timeout = 5000) : base(ip, port, timeout) { }
        ...
        private OperateResult<byte[]> ReadVar(string varName)
        {
            byte[] nameBytes = System.Text.Encoding.ASCII.GetBytes(varName);
            ...
            if (response.Length < 4) return OperateResult<byte[]>.Failed("PCDK 响应过短");
            byte[] data = new byte[response.Length - 4];
            Buffer.BlockCopy(response, 4, data, 0, data.Length);
            return OperateResult<byte[]>.Success(data);
        }

        private OperateResult WriteVar(string varName, byte[] data)
        {
            byte[] nameBytes = System.Text.Encoding.ASCII.GetBytes(varName);
        ...
        public override OperateResult<bool> ReadBool(string address) { var r = ReadVar(address); if (!r.IsSuccess) return OperateResult<bool>.Failed(r.Message, r.ErrorCode); return OperateResult<bool>.Success(r.Content.Length > 0 && r.Content[0] != 0); }
        public override OperateResult<short> ReadInt16(string address) { var r = ReadVar(address); if (!r.IsSuccess) return OperateResult<short>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 2) return OperateResult<short>.Failed("数据不足"); return OperateResult<short>.Success((short)(r.Content[0] | (r.Content[1] << 8))); }
        ...
        public override OperateResult<int> ReadInt32(string address) { var r = ReadVar(address); if (!r.IsSuccess) return OperateResult<int>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 4) return OperateResult<int>.Failed("数据不足"); return OperateResult<int>.Success(r.Content[0] | (r.Content[1] << 8) | (r.Content[2] << 16) | (r.Content[3] << 24)); }
        ...
        public override OperateResult<float> ReadFloat(string address) { var r = ReadVar(address); if (!r.IsSuccess) return OperateResult<float>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 4) return OperateResult<float>.Failed("数据不足"); return OperateResult<float>.Success(DataConverter.ToFloat(r.Content, 0)); }
        ...
        public override OperateResult<string> ReadString(string address, ushort length) { var r = ReadVar(address); if (!r.IsSuccess) return OperateResult<string>.Failed(r.Message, r.ErrorCode); return OperateResult<string>.Success(System.Text.Encoding.ASCII.GetString(r.Content, 0, Math.Min(length, r.Content.Length)).TrimEnd('\0')); }
        public override OperateResult<byte[]> ReadBytes(string address, ushort length) { return ReadVar(address); }

        public override OperateResult Write(string address, bool value) { return WriteVar(address, new byte[] { (byte)(value ? 1 : 0) }); }
        public override OperateResult Write(string address, short value) { return WriteVar(address, new byte[] { (byte)value, (byte)(value >> 8) }); }
        public override OperateResult Write(string address, ushort value) => Write(address, (short)value);
        public override OperateResult Write(string address, int value) { return WriteVar(address, new byte[] { (byte)value, (byte)(value >> 8), (byte)(value >> 16), (byte)(value >> 24) }); }
        ...
        public override OperateResult Write(string address, string value) { return WriteVar(address, System.Text.Encoding.ASCII.GetBytes(value)); }
        public override OperateResult Write(string address, byte[] data) { return WriteVar(address, data); }
