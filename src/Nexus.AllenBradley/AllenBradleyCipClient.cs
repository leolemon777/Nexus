...
{
    /// <summary>
    /// Allen-Bradley EtherNet/IP CIP 客户端 — 支持 ControlLogix / CompactLogix Tag 读写。
    ...
    /// </summary>
    public class AllenBradleyCipClient : IReadWriteDevice, IBatchReadWrite
    {
        ...
        /// <summary>端口号（默认 44818）。</summary>
        public int Port { get; }
        /// <summary>目标槽号（ControlLogix 背板槽号，默认 0）。</summary>
        ...
        public AllenBradleyCipClient(string ipAddress, int port = 44818, byte slot = 0, int timeout = 5000)
        {
        ...
        private short ToInt16LE(byte[] data, int offset = 0)
            => (short)(data[offset] | (data[offset + 1] << 8));

        private ushort ToUInt16LE(byte[] data, int offset = 0)
            => (ushort)(data[offset] | (data[offset + 1] << 8));

        private int ToInt32LE(byte[] data, int offset = 0)
            => data[offset] | (data[offset + 1] << 8) | (data[offset + 2] << 16) | (data[offset + 3] << 24);

        private uint ToUInt32LE(byte[] data, int offset = 0)
            => (uint)(data[offset] | (data[offset + 1] << 8) | (data[offset + 2] << 16) | (data[offset + 3] << 24));

        private unsafe float ToFloatLE(byte[] data, int offset = 0)
        {
            int v = ToInt32LE(data, offset);
            return *(float*)&v;
        }

        private unsafe double ToDoubleLE(byte[] data, int offset = 0)
        {
        ...
        private OperateResult<byte[]> SendEnip(EnipCommand command, byte[] data)
        {
            try
            {
                lock (_lock)
                {
                    ...
                    // ENIP Header: Command(2) + Length(2) + SessionHandle(4) + Status(4) + SenderContext(8) + Options(4) = 24 bytes
                    ...
                    // Session handle (already set from _sessionHandle)
                    ...
                    // Status = 0, SenderContext = 0, Options = 0 (already zero)
                    ...
                    // 提取 session handle（RegisterSession 响应）
        ...
        private byte[] EncodeTagPath(string tagName)
        {
            ...
            {
                // 解析数组索引和嵌套 tag（支持 Tag.Member 和 Tag[0] 语法）
        ...
        private const byte CipReadFragmented = 0x52;
        private const byte CipWriteService = 0x4D;
        private const byte CipWriteFragmented = 0x53;
        private const byte CipMultipleService = 0x0A;
        ...
        private const ushort CipTypeStruct = 0x02A0;

        // ═══════════════════════════════════════════
        //  CIP 读 Tag（基础）
        // ═══════════════════════════════════════════

        /// <summary>读取 CIP Tag 原始数据。</summary>
        private OperateResult<byte[]> ReadTagRaw(string tagName, ushort elements = 1)
        {
            byte[] path = EncodeTagPath(tagName);

            // CIP Read Request: Service(1) + PathSize(1) + Path(n) + Elements(2)
        ...
        /// <summary>写入 CIP Tag 原始数据。</summary>
        private OperateResult WriteTagRaw(string tagName, ushort dataType, byte[] data, ushort elements = 1)
        {
            byte[] path = EncodeTagPath(tagName);

            // CIP Write Request: Service(1) + PathSize(1) + Path(n) + Type(2) + Elements(2) + Data(n)
            int pathWords = (path.Length + 1) / 2;
        ...
        // ═══════════════════════════════════════════
        //  Tag Fragmented Read / Write
        // ═══════════════════════════════════════════

        /// <summary>
        /// 分段读取大 Tag 数据（CIP Read Fragmented, 0x52）。
        /// <para>自动循环读取直到数据全部返回，适用于超出单次 PDU 的大数组或结构体。</para>
        /// </summary>
        /// <param name="tagName">Tag 名称</param>
        /// <param name="offset">起始字节偏移</param>
        /// <param name="count">要读取的字节数（0 = 读取全部）</param>
        public OperateResult<byte[]> ReadTagFragmented(string tagName, uint offset = 0, uint count = 0)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    // CIP Read Fragmented: Service(1) + PathSize(1) + Path(n) + Offset(4)
                    ...
                    // 首次返回包含 Type(2) + Count(4) + Data；后续分片只含 Data
        ...
        /// <summary>
        /// 分段写入大 Tag 数据（CIP Write Fragmented, 0x53）。
        /// <para>自动分片发送，适用于超出单次 PDU 的大数组或结构体。</para>
        /// </summary>
        /// <param name="tagName">Tag 名称</param>
        /// <param name="data">要写入的完整数据</param>
        /// <param name="offset">起始字节偏移</param>
        /// <param name="dataType">CIP 数据类型（默认 DINT）</param>
        public OperateResult WriteTagFragmented(string tagName, byte[] data, uint offset = 0, ushort dataType = CipTypeDint)
        {
            try
            {
                ...
                while (dataOffset < data.Length)
                {
                    ...
                    // CIP Write Fragmented: Service(1) + PathSize(1) + Path(n) + Type(2) + Count(4) + Offset(4) + Data(n)
                    ...
                    if (isFirst)
                    {
                        // Count = total data length for first fragment
        ...
        /// <summary>
        /// 批量读取多个 Tag（CIP Multiple Service Packet, 0x0A）。
        /// <para>将多个 Read Tag 服务请求打包到单个 CIP 消息中，减少网络往返。</para>
        /// </summary>
        /// <param name="tagNames">要读取的 Tag 名称列表</param>
        public OperateResult<Dictionary<string, byte[]>> BatchReadTags(IEnumerable<string> tagNames)
        {
            ...
            try
            {
                ...
                foreach (string tag in tagList)
                {
                    ...
                    int p = 2 + pathWords * 2;
                    req[p] = 0x01; // 1 element
                    req[p + 1] = 0x00;
                    serviceRequests.Add(req);
                }

                // 计算 Multiple Service Packet
                ...
                // CIP Multiple Service: Service(1) + PathSize(1) + Path(2) + Count(2) + Offsets(n) + Services(n)
                ...
                for (int i = 0; i < respCount && i < tagList.Count; i++)
                {
                    ...
                    if (svcLen >= 4)
                    {
                        ...
                        if (svcStatus == 0)
                        {
                            // Skip reply service(1) + reserved(1) + status(1) + extStatusSize(1) = 4 bytes
        ...
        /// <summary>
        /// 读取 CIP STRING 类型 Tag（长度前缀格式）。
        /// <para>CIP STRING 结构: DINT(4字节长度) + ASCII 字符数据</para>
        /// </summary>
        ...
        /// <summary>
        /// 写入 CIP STRING 类型 Tag。
        /// <para>CIP STRING 结构: DINT(4字节长度) + ASCII 字符数据</para>
        /// </summary>
        public OperateResult WriteTagString(string tagName, string value)
        {
            byte[] strBytes = System.Text.Encoding.ASCII.GetBytes(value ?? string.Empty);
        ...
        private static string CipStatusMessage(byte status) => status switch
        {
            0x01 => "连接失败",
            0x02 => "资源不可用",
            0x03 => "无效参数值",
        ...
        /// <summary>
        /// 读取控制器标识 (ListIdentity) — 获取 PLC 设备信息。
        /// <para>返回设备名称、厂商、设备类型、IP 地址等。</para>
        /// </summary>
        public OperateResult<CipIdentity> ReadDeviceIdentity()
        {
            ...
            // ENIP ListIdentity 响应: ItemCount(2) + ItemType(2) + ItemLength(2) + EncapsulationVersion(2)
        ...
        /// <summary>
        /// 读取控制器标签值 — 读取 "Program:Main" 或 Controller scope 标签。
        /// </summary>
        /// <param name="tagName">标签名，如 "MyTag" 或 "Program:Main.MyTag"。</param>
        public OperateResult<object?> ReadTagValue(string tagName)
        {
            var r = ReadTagRaw(tagName);
            if (!r.IsSuccess) return OperateResult<object?>.Failed(r.Message, r.ErrorCode);
            ...
            int dataOff = 4;
            if (r.Content.Length <= dataOff)
                return OperateResult<object?>.Success((object?)null);

            byte[] tagData = new byte[r.Content.Length - dataOff];
            Buffer.BlockCopy(r.Content, dataOff, tagData, 0, tagData.Length);
    ...
    public sealed class CipIdentity
    {
        ...
        /// <summary>厂商 ID (Rockwell=1)。</summary>
