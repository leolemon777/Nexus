...
{
    /// <summary>
    /// FANUC Robot SocketMessage 连接池 — 复用持久 TCP 连接，降低寄存器、IO 和状态轮询的建连成本。
    /// </summary>
    public sealed class FanucRobotConnectionPool : IDisposable
    {
        private readonly ConnectionPool<FanucRobotClient> _pool;
        ...
        public FanucRobotConnectionPool(
            string ip,
            int port = 60008,
            int timeout = 5000,
            int maxPoolSize = 5,
            ...
            ILogger? logger = null)
        {
            ...
            _pool = new ConnectionPool<FanucRobotClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<FanucRobotClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            FanucRobotClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"FANUC Robot 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<FanucRobotClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"FANUC Robot 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<FanucRobotClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<FanucRobotClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        public OperateResult<int> ReadNumericRegister(int regIndex) => Execute(c => c.ReadNumericRegister(regIndex));
        public OperateResult WriteNumericRegister(int regIndex, int value) => Execute(c => c.WriteNumericRegister(regIndex, value));
        public OperateResult<double[]> ReadPositionRegister(int regIndex) => Execute(c => c.ReadPositionRegister(regIndex));
        public OperateResult WritePositionRegister(int regIndex, double[] values) => Execute(c => c.WritePositionRegister(regIndex, values));
        public OperateResult<string> ReadStringRegister(int regIndex) => Execute(c => c.ReadStringRegister(regIndex));
        public OperateResult WriteStringRegister(int regIndex, string value) => Execute(c => c.WriteStringRegister(regIndex, value));
        public OperateResult<bool> ReadDigitalInput(int index) => Execute(c => c.ReadDigitalInput(index));
        public OperateResult<bool> ReadDigitalOutput(int index) => Execute(c => c.ReadDigitalOutput(index));
        public OperateResult WriteDigitalOutput(int index, bool value) => Execute(c => c.WriteDigitalOutput(index, value));
        public OperateResult<int> ReadGroupInput(int index) => Execute(c => c.ReadGroupInput(index));
        public OperateResult<int> ReadGroupOutput(int index) => Execute(c => c.ReadGroupOutput(index));
        public OperateResult WriteGroupOutput(int index, int value) => Execute(c => c.WriteGroupOutput(index, value));
        public OperateResult<double[]> ReadRobotPosition() => Execute(c => c.ReadRobotPosition());
        public OperateResult<FanucRobotStatus> ReadRobotStatus() => Execute(c => c.ReadRobotStatus());
        public OperateResult SendString(string message) => Execute(c => c.SendString(message));
        ...
        private FanucRobotClient CreateClient()
        {
            var client = new FanucRobotClient(_ip, _port, _timeout);
