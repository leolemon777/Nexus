...
{
    /// <summary>
    /// Phoenix Contact AXC 虚拟 PLC 服务器 — 模拟 Modbus TCP 兼容协议（AXC 作为 Server）。
    /// <para>用于集成测试，无需真实 Phoenix PLC。</para>
    /// <para>支持 FC01/02/03/04/05/06/0F/10 功能码。</para>
    /// </summary>
    public class PhoenixVirtualServer : IDisposable
    {
        ...
        public PhoenixVirtualServer(int port = 5020)
        {
            Port = port;
        }
        ...
        public void SetInputRegister(ushort address, ushort value) { lock (_memLock) _inputRegisters[address] = value; }
