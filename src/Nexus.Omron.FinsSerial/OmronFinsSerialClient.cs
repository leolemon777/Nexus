...
{
    /// <summary>
    /// 欧姆龙 FINS 串口客户端 — 通过 RS-232/485 与欧姆龙 PLC 通信。
    /// <para>基于 FINS (Factory Interface Network Service) 协议的串口封装。</para>
    /// <para>支持地址: D, W, H, CIO, A, E</para>
    /// </summary>
    public class OmronFinsSerialClient : SerialDeviceBase, IBatchReadWrite, ISubscribeDevice
    {
        public byte LocalNode { get; set; } = 0;
        public byte RemoteNode { get; set; } = 0;
        public Endianness ByteOrder { get; set; } = Endianness.BigEndian;

        private readonly FinsSerialAddressParser _parser = new FinsSerialAddressParser();

        public OmronFinsSerialClient(ISerialPort port, int timeout = 5000) : base(port, timeout) { }
        ...
        private OperateResult<byte[]> SendFinsCommand(byte serviceClass, byte serviceCode, byte[] data)
        {
            ...
            var result = base.SendAndReceive(finsFrame);
        ...
        public override OperateResult<long> ReadInt64(string address)
        {
            ...
            return OperateResult<long>.Success(((long)r.Content[0] << 56) | ((long)r.Content[1] << 48) | ((long)r.Content[2] << 40) | ((long)r.Content[3] << 32) | ((long)r.Content[4] << 24) | ((long)r.Content[5] << 16) | ((long)r.Content[6] << 8) | r.Content[7]);
        }
        ...
        public override OperateResult<byte[]> ReadBytes(string address, ushort length)
        {
            ...
            byte[] data = new byte[length]; Buffer.BlockCopy(r.Content, 0, data, 0, Math.Min(length, r.Content.Length)); return OperateResult<byte[]>.Success(data);
        }
        ...
        public override OperateResult Write(string address, bool value)
        {
            ...
            if (value) b |= (byte)(1 << addr.BitOffset); else b &= (byte)~(1 << addr.BitOffset);
