...
{
    /// <summary>
    /// KUKA EKI 连接池 — 复用持久 XML/TCP 连接，降低变量读写和状态查询的建连成本。
    /// </summary>
    public sealed class KukaEkiConnectionPool : IDisposable
    {
        private readonly ConnectionPool<KukaEkiClient> _pool;
            ...
            ILogger? logger = null)
        {
            ...
            _pool = new ConnectionPool<KukaEkiClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<KukaEkiClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            KukaEkiClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"KUKA EKI 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<KukaEkiClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"KUKA EKI 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<KukaEkiClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<KukaEkiClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        public OperateResult<KukaCartesianPosition> ReadPosition() => Execute(c => c.ReadPosition());
        public OperateResult<double[]> ReadAxisPosition() => Execute(c => c.ReadAxisPosition());
        public OperateResult<KukaProgramState> ReadProgramState() => Execute(c => c.ReadProgramState());
        public OperateResult StartProgram(string programName) => Execute(c => c.StartProgram(programName));
        public OperateResult StopMotion() => Execute(c => c.StopMotion());
        public OperateResult Jog(int axis, double velocity) => Execute(c => c.Jog(axis, velocity));
        ...
        private KukaEkiClient CreateClient()
        {
            var client = new KukaEkiClient(_ip, _port, _timeout);
