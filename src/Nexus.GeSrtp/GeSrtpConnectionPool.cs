...
{
    /// <summary>
    /// GE SRTP 连接池 — 复用 TCP 连接，适合高频寄存器轮询和批量读写。
    /// </summary>
    public sealed class GeSrtpConnectionPool : IDisposable
    {
        private readonly ConnectionPool<GeSrtpClient> _pool;
            ...
            ILogger? logger = null)
        {
            ...
            _key = $"{_ipAddress}:{_port}:{_timeout}";
            _pool = new ConnectionPool<GeSrtpClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<GeSrtpClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            GeSrtpClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"GE SRTP 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<GeSrtpClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"GE SRTP 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<GeSrtpClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<GeSrtpClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        public OperateResult<DateTime> ReadDateTime() => Execute(c => c.ReadDateTime());
        public OperateResult<string> ReadProgramName() => Execute(c => c.ReadProgramName());
        public OperateResult<string> ReadPlcStatus() => Execute(c => c.ReadPlcStatus());

        public void Clear() => _pool.Clear();
        public void Dispose() => _pool.Dispose();

        private GeSrtpClient CreateClient()
        {
            var client = new GeSrtpClient(_ipAddress, _port, _timeout);
