...
{
    /// <summary>
    /// YASKAWA Memobus TCP 连接池 — 复用持久连接，降低高频读写和批量操作的建连成本。
    /// </summary>
    public sealed class MemobusConnectionPool : IDisposable
    {
        private readonly ConnectionPool<MemobusClient> _pool;
            ...
            ILogger? logger = null)
        {
            ...
            _key = $"{_ip}:{_port}:{_timeout}:{_cpuTo}:{_cpuFrom}";
            _pool = new ConnectionPool<MemobusClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<MemobusClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            MemobusClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"YASKAWA Memobus 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<MemobusClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"YASKAWA Memobus 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<MemobusClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<MemobusClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        private MemobusClient CreateClient()
        {
            var client = new MemobusClient(_ip, _port, _timeout)
            {
                CpuTo = _cpuTo,
                CpuFrom = _cpuFrom
            };
