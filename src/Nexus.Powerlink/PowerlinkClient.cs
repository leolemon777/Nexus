...
namespace Nexus.Powerlink
{
    public sealed class PowerlinkAddress : IDataAddress
    {
        ...
        public PowerlinkAddress(string original, ushort objectIndex, byte subIndex, byte nodeId = 0)
        {
            Original = original;
    ...
    public sealed class PowerlinkAddressParser : IAddressParser<PowerlinkAddress>
    {
        public PowerlinkAddress Parse(string address)
        {
            if (string.IsNullOrWhiteSpace(address)) throw new AddressParseException(address, "地址不能为空");
            string original = address;
            address = address.Trim();
            string[] parts = address.Split('.');
            if (parts.Length == 3) return new PowerlinkAddress(original, ushort.Parse(parts[0]), byte.Parse(parts[1]), byte.Parse(parts[2]));
            if (parts.Length == 2) return new PowerlinkAddress(original, ushort.Parse(parts[0]), byte.Parse(parts[1]));
            if (parts.Length == 1) return new PowerlinkAddress(original, ushort.Parse(parts[0]), 0);
            throw new AddressParseException(address, "POWERLINK 地址格式: [node.]index.subindex");
        }
        public bool TryParse(string address, out PowerlinkAddress? parsed) { try { parsed = Parse(address); return true; } catch { parsed = null; return false; } }
    }

    /// <summary>
    /// POWERLINK 客户端 — B&R POWERLINK 工业以太网协议。
    /// <para>通过 POWERLINK 网关访问对象字典。</para>
    /// <para>地址格式: [node.]index.subindex</para>
    /// </summary>
    public class PowerlinkClient : TcpDeviceBase, IBatchReadWrite
    {
        public byte DefaultNodeId { get; set; } = 0;
        private readonly PowerlinkAddressParser _parser = new PowerlinkAddressParser();

        public PowerlinkClient(string ip, int port = 34980, int timeout = 5000) : base(ip, port, timeout) { }
        ...
        {
            byte[] request = new byte[] { 0x01, nodeId, (byte)(index >> 8), (byte)(index & 0xFF), subIndex, 0, (byte)(size >> 8), (byte)(size & 0xFF) };
            ...
            if (response.Length < 4) return OperateResult<byte[]>.Failed("POWERLINK 响应过短");
            uint error = (uint)((response[0] << 24) | (response[1] << 16) | (response[2] << 8) | response[3]);
            if (error != 0) return OperateResult<byte[]>.Failed($"POWERLINK 错误: 0x{error:X8}");
            byte[] data = new byte[response.Length - 4];
            Buffer.BlockCopy(response, 4, data, 0, data.Length);
        ...
        private OperateResult WriteOd(byte nodeId, ushort index, byte subIndex, byte[] data)
        {
            ...
            request[0] = 0x02; request[1] = nodeId;
            request[2] = (byte)(index >> 8); request[3] = (byte)(index & 0xFF);
            request[4] = subIndex; request[5] = 0;
            request[6] = (byte)(data.Length >> 8); request[7] = (byte)(data.Length & 0xFF);
        ...
        private PowerlinkAddress ParseAddr(string address) { var addr = _parser.Parse(address); return addr.NodeId == 0 ? new PowerlinkAddress(addr.Original, addr.ObjectIndex, addr.SubIndex, DefaultNodeId) : addr; }

        public override OperateResult<bool> ReadBool(string address) { var addr = ParseAddr(address); var r = ReadOd(addr.NodeId, addr.ObjectIndex, addr.SubIndex, 1); if (!r.IsSuccess) return OperateResult<bool>.Failed(r.Message, r.ErrorCode); return OperateResult<bool>.Success(r.Content.Length > 0 && r.Content[0] != 0); }
        public override OperateResult<short> ReadInt16(string address) { var addr = ParseAddr(address); var r = ReadOd(addr.NodeId, addr.ObjectIndex, addr.SubIndex, 2); if (!r.IsSuccess) return OperateResult<short>.Failed(r.Message, r.ErrorCode); if (r.Content.Length < 2) return OperateResult<short>.Failed("数据不足"); return OperateResult<short>.Success((short)(r.Content[0] | (r.Content[1] << 8))); }
        ...
        public override OperateResult<string> ReadString(string address, ushort length) { var addr = ParseAddr(address); var r = ReadOd(addr.NodeId, addr.ObjectIndex, addr.SubIndex, length); if (!r.IsSuccess) return OperateResult<string>.Failed(r.Message, r.ErrorCode); return OperateResult<string>.Success(System.Text.Encoding.ASCII.GetString(r.Content, 0, Math.Min(length, r.Content.Length)).TrimEnd('\0')); }
        public override OperateResult<byte[]> ReadBytes(string address, ushort length) { var addr = ParseAddr(address); return ReadOd(addr.NodeId, addr.ObjectIndex, addr.SubIndex, length); }

        public override OperateResult Write(string address, bool value) { var addr = ParseAddr(address); return WriteOd(addr.NodeId, addr.ObjectIndex, addr.SubIndex, new byte[] { (byte)(value ? 1 : 0) }); }
        public override OperateResult Write(string address, short value) { var addr = ParseAddr(address); return WriteOd(addr.NodeId, addr.ObjectIndex, addr.SubIndex, new byte[] { (byte)value, (byte)(value >> 8) }); }
        public override OperateResult Write(string address, ushort value) => Write(address, (short)value);
        public override OperateResult Write(string address, int value) { var addr = ParseAddr(address); return WriteOd(addr.NodeId, addr.ObjectIndex, addr.SubIndex, new byte[] { (byte)value, (byte)(value >> 8), (byte)(value >> 16), (byte)(value >> 24) }); }
        ...
        public override OperateResult Write(string address, string value) { var addr = ParseAddr(address); return WriteOd(addr.NodeId, addr.ObjectIndex, addr.SubIndex, System.Text.Encoding.ASCII.GetBytes(value)); }
        public override OperateResult Write(string address, byte[] data) { var addr = ParseAddr(address); return WriteOd(addr.NodeId, addr.ObjectIndex, addr.SubIndex, data); }
