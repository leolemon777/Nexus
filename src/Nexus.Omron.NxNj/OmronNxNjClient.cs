...
{
    /// <summary>
    /// Omron NX/NJ 客户端 — FINS over Ethernet，支持新一代欧姆龙控制器。
    /// <para>支持地址: D(数据存储器), W(工作), H(保持), CIO, A(辅助), E(扩展), I(索引)</para>
    /// </summary>
    public class OmronNxNjClient : TcpDeviceBase, IBatchReadWrite, ISubscribeDevice
    {
        public byte LocalNode { get; set; } = 0;
        public byte RemoteNode { get; set; } = 0;
        public Endianness ByteOrder { get; set; } = Endianness.BigEndian;

        private readonly OmronNxNjAddressParser _parser = new OmronNxNjAddressParser();

        public OmronNxNjClient(string ip, int port = 9600, int timeout = 5000)
        ...
        private OperateResult<byte[]> SendFinsCommand(byte serviceClass, byte serviceCode, byte[] data)
        {
            ...
            finsFrame[10] = serviceClass; finsFrame[11] = serviceCode;
            ...
            byte[] pdu = new byte[response.Length - 14];
            Buffer.BlockCopy(response, 14, pdu, 0, pdu.Length);
        ...
        private OperateResult<byte[]> ReadWords(string areaCode, ushort startWord, ushort count)
        {
            ...
            var r = SendFinsCommand(0x01, 0x01, data);
            if (!r.IsSuccess) return r;
            byte[] result = new byte[count * 2];
            Buffer.BlockCopy(r.Content, 0, result, 0, Math.Min(result.Length, r.Content.Length));
        ...
        private OperateResult WriteWords(string areaCode, ushort startWord, byte[] wordData)
        {
            ...
            var r = SendFinsCommand(0x01, 0x02, data);
        ...
        public override OperateResult<bool> ReadBool(string address)
        {
            ...
            var r = ReadWords(addr.AreaCode, addr.WordAddress, 1);
        ...
        {
            var addr = _parser.Parse(address);
            var r = ReadWords(addr.AreaCode, addr.WordAddress, 2);
        ...
        {
            var addr = _parser.Parse(address);
            var r = ReadWords(addr.AreaCode, addr.WordAddress, 4);
        ...
        public override OperateResult<string> ReadString(string address, ushort length)
        {
            ...
            var r = ReadWords(addr.AreaCode, addr.WordAddress, regCount);
        ...
        {
            var addr = _parser.Parse(address);
            var current = ReadWords(addr.AreaCode, addr.WordAddress, 1);
            ...
            return WriteWords(addr.AreaCode, addr.WordAddress, new byte[] { b, current.Content[1] });
        }

        public override OperateResult Write(string address, short value)
        {
            var addr = _parser.Parse(address);
            return WriteWords(addr.AreaCode, addr.WordAddress, new byte[] { (byte)(value >> 8), (byte)value });
        }
        ...
        {
            var addr = _parser.Parse(address);
            return WriteWords(addr.AreaCode, addr.WordAddress, new byte[] { (byte)(value >> 24), (byte)(value >> 16), (byte)(value >> 8), (byte)value });
        }
        ...
        {
            var addr = _parser.Parse(address);
            return WriteWords(addr.AreaCode, addr.WordAddress, new byte[] {
        ...
        public override OperateResult Write(string address, string value)
        {
            ...
            return WriteWords(addr.AreaCode, addr.WordAddress, strData);
        }
        ...
        public override OperateResult Write(string address, byte[] data)
        {
            ...
            return WriteWords(addr.AreaCode, addr.WordAddress, data);
        }
        ...
        private readonly Dictionary<string, NxNjSub> _subs = new Dictionary<string, NxNjSub>();
        private readonly object _subLock = new object();
        private Timer? _timer;
        private bool _monitoring;

        private class NxNjSub { public string Address = ""; public string DataType = "Int16"; public object? LastValue; }
        ...
        { lock (_subLock) { _subs[address] = new NxNjSub { Address = address, DataType = dataType }; } }
        ...
        private void Poll(object? state)
        {
            ...
            try
            {
                List<NxNjSub> entries;
                lock (_subLock) { entries = new List<NxNjSub>(_subs.Values); }
