...
namespace Nexus.CcLinkIEField
{
    /// <summary>
    /// CC-Link IE Field 客户端 — 三菱 CC-Link IE Field 网络。
    /// <para>基于 MC 协议（二进制格式），支持 CC-Link IE Field 设备。</para>
    /// <para>地址格式与 CC-Link IE 相同: D, R, W, B 等</para>
    /// </summary>
    public class CcLinkIEFieldClient : TcpDeviceBase, IBatchReadWrite
    {
        ...
        private readonly CcLinkIe.CcLinkIeAddressParser _parser = new CcLinkIe.CcLinkIeAddressParser();

        public CcLinkIEFieldClient(string ip, int port = 4999, int timeout = 5000) : base(ip, port, timeout) { }
        protected override int ResponseHeaderLength => 9;
        protected override int GetResponsePayloadLength(byte[] header) => 0;

        private ushort DeviceCode(CcLinkIe.CcLinkIeDeviceType type) => type switch
        {
            CcLinkIe.CcLinkIeDeviceType.R => 0xAF, CcLinkIe.CcLinkIeDeviceType.WR => 0xB4,
            CcLinkIe.CcLinkIeDeviceType.LR => 0xA0, CcLinkIe.CcLinkIeDeviceType.SW => 0xB5,
            CcLinkIe.CcLinkIeDeviceType.SB => 0xA1, CcLinkIe.CcLinkIeDeviceType.DX => 0x9C,
            CcLinkIe.CcLinkIeDeviceType.DY => 0x9D, CcLinkIe.CcLinkIeDeviceType.W => 0xB4,
            CcLinkIe.CcLinkIeDeviceType.B => 0xA0, CcLinkIe.CcLinkIeDeviceType.D => 0xA8,
            _ => 0xA8
        };
        ...
        private byte[] BuildMcFrame(ushort command, ushort subCommand, byte[] data)
        {
            ...
            ushort sum = 0; for (int i = 0; i < 11 + dataLen; i++) sum += frame[i];
            frame[11 + dataLen] = (byte)(sum & 0xFF); frame[12 + dataLen] = (byte)(sum >> 8);
            return frame;
        }
        ...
        private OperateResult<byte[]> SendMc(ushort command, ushort subCommand, byte[] data)
        {
            ...
            if (response.Length < 11) return OperateResult<byte[]>.Failed("CC-Link IE Field 响应过短");
            ushort completionCode = (ushort)(response[9] | (response[10] << 8));
            if (completionCode != 0) return OperateResult<byte[]>.Failed($"CC-Link IE Field 错误: 0x{completionCode:X4}", completionCode);
            byte[] pdu = new byte[response.Length - 11];
            Buffer.BlockCopy(response, 11, pdu, 0, pdu.Length);
            return OperateResult<byte[]>.Success(pdu);
        }

        public override OperateResult<bool> ReadBool(string address) { var addr = _parser.Parse(address); byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, (byte)DeviceCode(addr.DeviceType), 1, 0 }; var r = SendMc(0x0401, 0x0001, data); if (!r.IsSuccess) return OperateResult<bool>.Failed(r.Message, r.ErrorCode); return OperateResult<bool>.Success((r.Content[0] & 0x01) != 0); }
        public override OperateResult<short> ReadInt16(string address) { var addr = _parser.Parse(address); byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, (byte)DeviceCode(addr.DeviceType), 1, 0 }; var r = SendMc(0x0401, 0x0000, data); if (!r.IsSuccess) return OperateResult<short>.Failed(r.Message, r.ErrorCode); return OperateResult<short>.Success((short)(r.Content[0] | (r.Content[1] << 8))); }
        ...
        public override OperateResult<int> ReadInt32(string address) { var addr = _parser.Parse(address); byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, (byte)DeviceCode(addr.DeviceType), 2, 0 }; var r = SendMc(0x0401, 0x0000, data); if (!r.IsSuccess) return OperateResult<int>.Failed(r.Message, r.ErrorCode); return OperateResult<int>.Success(r.Content[0] | (r.Content[1] << 8) | (r.Content[2] << 16) | (r.Content[3] << 24)); }
        ...
        public override OperateResult<float> ReadFloat(string address) { var addr = _parser.Parse(address); byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, (byte)DeviceCode(addr.DeviceType), 2, 0 }; var r = SendMc(0x0401, 0x0000, data); if (!r.IsSuccess) return OperateResult<float>.Failed(r.Message, r.ErrorCode); return OperateResult<float>.Success(DataConverter.ToFloat(r.Content, 0)); }
        ...
        public override OperateResult<string> ReadString(string address, ushort length) { var addr = _parser.Parse(address); ushort regCount = (ushort)((length + 1) / 2); byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, (byte)DeviceCode(addr.DeviceType), (byte)(regCount & 0xFF), (byte)(regCount >> 8) }; var r = SendMc(0x0401, 0x0000, data); if (!r.IsSuccess) return OperateResult<string>.Failed(r.Message, r.ErrorCode); return OperateResult<string>.Success(DataConverter.ToString(r.Content, 0, length)); }
        public override OperateResult<byte[]> ReadBytes(string address, ushort length) { var addr = _parser.Parse(address); ushort regCount = (ushort)((length + 1) / 2); byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, (byte)DeviceCode(addr.DeviceType), (byte)(regCount & 0xFF), (byte)(regCount >> 8) }; var r = SendMc(0x0401, 0x0000, data); if (!r.IsSuccess) return OperateResult<byte[]>.Failed(r.Message, r.ErrorCode); byte[] result = new byte[length]; Buffer.BlockCopy(r.Content, 0, result, 0, Math.Min(length, r.Content.Length)); return OperateResult<byte[]>.Success(result); }

        private OperateResult WriteWords(CcLinkIe.CcLinkIeAddress addr, byte[] wordData) { ushort count = (ushort)(wordData.Length / 2); byte[] data = new byte[6 + wordData.Length]; data[0] = (byte)(addr.StartAddress & 0xFF); data[1] = (byte)(addr.StartAddress >> 8); data[2] = 0; data[3] = (byte)DeviceCode(addr.DeviceType); data[4] = (byte)(count & 0xFF); data[5] = (byte)(count >> 8); Buffer.BlockCopy(wordData, 0, data, 6, wordData.Length); var r = SendMc(0x1401, 0x0000, data); return r.IsSuccess ? OperateResult.Success() : OperateResult.Failed(r.Message, r.ErrorCode); }
        public override OperateResult Write(string address, bool value) { var addr = _parser.Parse(address); byte[] data = new byte[] { (byte)(addr.StartAddress & 0xFF), (byte)(addr.StartAddress >> 8), 0, (byte)DeviceCode(addr.DeviceType), 1, 0, (byte)(value ? 0x01 : 0x00) }; var r = SendMc(0x1401, 0x0001, data); return r.IsSuccess ? OperateResult.Success() : OperateResult.Failed(r.Message, r.ErrorCode); }
