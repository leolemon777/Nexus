...
{
    /// <summary>
    /// Modbus RTU-over-TCP 连接池 — 复用承载 RTU ADU 的持久 TCP 连接。
    /// </summary>
    public sealed class ModbusRtuOverTcpConnectionPool : IDisposable
    {
        private readonly ConnectionPool<ModbusRtuOverTcpClient> _pool;
        ...
        public ModbusRtuOverTcpConnectionPool(
            string ip,
            int port = 502,
            byte station = 1,
            int timeout = 5000,
            ...
            ILogger? logger = null)
        {
            ...
            _pool = new ConnectionPool<ModbusRtuOverTcpClient>(
                CreateClient,
                maxPoolSize,
                idleTimeout,
                cleanupInterval);
        ...
        public OperateResult<T> Execute<T>(Func<ModbusRtuOverTcpClient, OperateResult<T>> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));

            ModbusRtuOverTcpClient? client = null;
            try
            {
            ...
            {
                return OperateResult<T>.Failed($"Modbus RTU-over-TCP 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public OperateResult Execute(Func<ModbusRtuOverTcpClient, OperateResult> operation)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
            ...
            {
                return OperateResult.Failed($"Modbus RTU-over-TCP 连接池操作失败: {ex.Message}");
            }
            finally
            {
                if (client != null)
        ...
        public async Task<OperateResult<T>> ExecuteAsync<T>(
            Func<ModbusRtuOverTcpClient, Task<OperateResult<T>>> operation,
        ...
        public async Task<OperateResult> ExecuteAsync(
            Func<ModbusRtuOverTcpClient, Task<OperateResult>> operation,
            CancellationToken cancellationToken = default)
        {
            if (operation == null) throw new ArgumentNullException(nameof(operation));
        ...
        private ModbusRtuOverTcpClient CreateClient()
        {
            var client = new ModbusRtuOverTcpClient(_ip, _port, _station, _timeout)
            {
