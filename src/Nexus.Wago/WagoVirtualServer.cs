...
{
    /// <summary>
    /// WAGO 虚拟 PLC 服务器 — 模拟 Modbus TCP 兼容协议（WAGO 750/PFC 作为 Server）。
    /// <para>用于集成测试，无需真实 WAGO PLC。</para>
    /// <para>支持 FC01/02/03/04/05/06/0F/10 功能码。</para>
    /// </summary>
    public class WagoVirtualServer : IDisposable
    {
        ...
        public WagoVirtualServer(int port = 5020)
        {
            Port = port;
        }
