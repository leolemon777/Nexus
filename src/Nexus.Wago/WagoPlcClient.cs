...
{
    /// <summary>
    /// WAGO 750/PFC PLC TCP 客户端 — 继承 TcpDeviceBase，复用连接管理。
    /// </summary>
    /// <remarks>
    /// <para>WAGO 750 以太网耦合器（750-3xx）与 PFC200 控制器原生作为 Modbus TCP Server。</para>
    /// <para>本客户端封装 IEC 61131-3 地址（%MW/%IW/%QX/%IX）到标准 Modbus TCP 的转换。</para>
    /// <para>默认端口 502（Modbus TCP 标准），默认站号 1。</para>
    /// <para><b>协议本质</b>：Modbus TCP variant（非 WAGO 私有协议）。地址映射来源：WAGO 750 耦合器手册 §4.5.6。</para>
    /// </remarks>
    public class WagoPlcClient : TcpDeviceBase, IBatchReadWrite
    {
        /// <summary>站号（Modbus Unit ID，默认 1）。</summary>
        public byte Station { get; set; } = 1;

        /// <summary>地址偏移约定（默认手册的 ZeroBased）。</summary>
        public WagoOffsetMode OffsetMode { get; set; } = WagoOffsetMode.ZeroBased;
        ...
        public WagoPlcClient(string ip, int port = 502, byte station = 1, int timeout = 5000)
        ...
        /// <summary>构建写单线圈 PDU (FC05)。</summary>
        ...
        /// <summary>构建写单寄存器 PDU (FC06)。</summary>
        public static byte[] BuildWriteSingleRegisterPdu(ushort addr, byte[] data)
        {
        ...
        /// <summary>构建写多寄存器 PDU (FC16)。</summary>
        ...
        {
            var addr = WagoAddress.TryParse(address, OffsetMode);
            if (addr == null)
                return OperateResult<byte[]>.Failed($"无法解析 WAGO 地址: {address}");
        ...
        public override OperateResult Write(string address, byte[] data)
        {
            ...
            var addr = WagoAddress.TryParse(address, OffsetMode);
            if (addr == null) return OperateResult.Failed($"无法解析 WAGO 地址: {address}");
