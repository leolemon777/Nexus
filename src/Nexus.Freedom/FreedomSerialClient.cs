...
{
    public class FreedomSerialClient : SerialDeviceBase, IBatchReadWrite
    {
        private int _stx;
        ...
        public FreedomSerialClient(ISerialPort serialPort, int timeout = 5000)
        ...
        protected new OperateResult<byte[]> SendAndReceive(byte[] request)
        {
            try
            {
                lock (_syncLock)
                {
                    ...
                    if (response.Length == 0)
                        return OperateResult<byte[]>.Failed("串口接收超时");
            ...
            catch (Exception ex)
            {
                ...
                var retry = TryReconnectAndRetry2(request);
            ...
            byte[] request, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                while (!linkedCts.Token.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        read = await Task.Run(() => Port.Read(buf, 0, buf.Length), linkedCts.Token).ConfigureAwait(false);
                    }
        ...
        private OperateResult<byte[]>? TryReconnectAndRetry2(byte[] request)
        {
            try
            {
                Log.Warn("尝试重连串口…");
                ...
                if (response.Length == 0)
                    return OperateResult<byte[]>.Failed("重连后接收超时");
