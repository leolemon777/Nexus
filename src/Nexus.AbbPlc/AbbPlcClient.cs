...
{
    /// <summary>
    /// ABB AC500 PLC TCP 客户端 — 继承 TcpDeviceBase，复用连接管理。
    /// </summary>
    /// <remarks>
    /// <para>ABB AC500 系列（PM571/PM58x/PM59x）原生作为 Modbus TCP Server。</para>
    /// <para>本客户端封装 IEC 61131-3 地址（%MW/%IW/%QX/%IX）到标准 Modbus TCP 的转换。</para>
    /// <para>默认端口 502，默认站号 1。</para>
    /// <para><b>协议本质</b>：Modbus TCP variant（非 ABB 私有协议）。</para>
    /// <para>注意：ABB 机器人协议见 <c>Nexus.ABB.EGM</c>（EGM）与 <c>Nexus.Robot.Abb</c>（IRC5 WebAPI），本库仅覆盖 PLC 产品线。</para>
    /// </remarks>
    public class AbbPlcClient : TcpDeviceBase, IBatchReadWrite
    {
        ...
        /// <summary>
        /// 创建 ABB AC500 PLC TCP 客户端实例。
        /// </summary>
        public AbbPlcClient(string ip, int port = 502, byte station = 1, int timeout = 5000)
        ...
        {
            var addr = AbbPlcAddress.TryParse(address);
            if (addr == null)
                return OperateResult<byte[]>.Failed($"无法解析 ABB 地址: {address}");
        ...
        public override OperateResult Write(string address, byte[] data)
        {
            ...
            var addr = AbbPlcAddress.TryParse(address);
            if (addr == null) return OperateResult.Failed($"无法解析 ABB 地址: {address}");
