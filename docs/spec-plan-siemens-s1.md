...
> 状态：**定稿，含 2026-08-22 SMART V3 勘误**。三路调研（2026-08-15）已返回并完成对账：
...
- SMART 并入本阶段（走标准 S7comm；现有兼容语法将 V 区映射为 DB1）：
  - 地址语法额外接受 `VB100`/`VW100`/`VD100`/`V100.3` → 自动映射 DB1
  - rack=0/slot=0（或 1）；单次读自动按 ≤200B 分片
  - **版本勘误**：S7-200 SMART V3 的 Put/Get Server 默认禁用；第三方 S7comm 客户端读写前，须在 STEP 7-Micro/WIN SMART V3 通讯设置中启用并下载配置。V2.8 及更早 CPU 使用与固件匹配的软件并按对应手册确认。
...
| 型号 | rack | slot | 说明 |
|---|---|---|---|
| S7-200 SMART | 0 | 0（1 亦可）| V3 需启用 Put/Get Server；V 语法按 DB1 兼容映射 |
| S7-300 | 0 | 2 | |
| S7-400 | 0 | 3 | 多机架会变 |
...
- **前置检查清单**（连接按钮旁链接，按型号显示）：1200/1500 五步（PUT/GET 勾选位置+保护等级+DB 非优化+重编译下载+固件版本）；300/400 三步；SMART 按 V2/V3 分支，V3 明确启用 Put/Get Server、下载配置，并建议将通信写入限制到保留 V 区
...
| SMART 固件差异（slot 0/1、V2/V3 安全设置） | 连接向导默认 0/0；失败自动重试 0/1 并提示；V3 未启用 Put/Get 时给出配置动作 |
