...
- **代码审查整改（2026-09-09）**：PoC 源码已按审查结论改动，上面这条 09-07 的验证结论**不再覆盖当前源码**，需重跑后更新。改动要点：端口改为绑定后直接交给 `Server::run_with()`（消除"释放→重绑"之间被抢占的窗口）；server 任务不再 `std::process::exit`，错误顺 `JoinHandle` 回主流程；`add_folder` 返回值改为检查（返回 `bool` 且非 `#[must_use]`，原先丢弃且无编译警告）；`read_values` 增加返回长度校验（`Session::read` 无长度保证，调用方按下标取值）；计数器任务与 client 事件循环保留 `JoinHandle` 并在收尾判定，关停超时结果不再丢弃；临时目录（含自签私钥）成功时清理、失败时保留现场并打出路径。PoC 已挂进 `scripts/build-evidence.candidate.json`（`opcua-poc-selfcheck`，跑的是完整自闭环而非只编译），门禁命令数由 6 增至 7。正式接入的硬约束见 `docs/opcua-blocked.md`。
- 待办（2026-09-09 起）：在 Windows 机器上重跑 `npm run fmt:opcua-poc`（整改是手写的，未过 rustfmt）、`npm run build:opcua-poc`、PoC 双次运行，通过后再更新上一条验证结论并重生成候选构建证据。
